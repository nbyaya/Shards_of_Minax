...
namespace Server.SkillHandlers
{
    ...
    public class Tracking
    {
        ...
        public static double GetStalkingBonus(Mobile tracker, Mobile target)
        {
            ...
            // If the tracker is a player, add bonus from TrackingDetection talent.
            if (tracker is PlayerMobile player)
            {
                var profile = player.AcquireTalents();
                bonus += profile.Talents[TalentID.TrackingDetection].Points * 0.5;
            }
    ...
    public class TrackWhoGump : Gump
    {
        ...
        public static void DisplayTo(bool success, Mobile from, int type)
        {
            ...
            // Apply passive bonus to range from TrackingRange talent.
            int baseRange = 10 + (int)(from.Skills[SkillName.Tracking].Value / 10);
            if (from is PlayerMobile player)
            {
                var profile = player.AcquireTalents();
                baseRange += profile.Talents[TalentID.TrackingRange].Points;
            }
            int range = baseRange;
            ...
            {
                if (type == 0)
                    from.SendLocalizedMessage(502991);
                else if (type == 1)
                    from.SendLocalizedMessage(502993);
                else
                    from.SendLocalizedMessage(502995);
            }
        }
		...
		private static bool CheckDifficulty(Mobile from, Mobile m)
		{
			...
			if (Core.ML && m.Race == Race.Elf)
				tracking /= 2; // Elves are harder to track

			int hiding = m.Skills[SkillName.Hiding].Fixed;
			int stealth = m.Skills[SkillName.Stealth].Fixed;
			int divisor = hiding + stealth;

			// Apply Tracking Skill Tree Bonuses
			if (from is PlayerMobile player)
			{
				var profile = player.AcquireTalents();
				
				// Tracking Detection boosts Detect Hidden effectiveness
				int trackingDetectionBonus = profile.Talents[TalentID.TrackingDetection].Points;
				detectHidden += trackingDetectionBonus * 10;

				// Small bonus to tracking skill if the player has high Tracking Detection
				tracking += trackingDetectionBonus * 5;
			}
        ...
        private static bool IsAnimal(Mobile m) { return (!m.Player && m.Body.IsAnimal); }
        private static bool IsMonster(Mobile m) { return (!m.Player && m.Body.IsMonster); }
        private static bool IsHumanNPC(Mobile m) { return (!m.Player && m.Body.IsHuman); }
        private static bool IsPlayer(Mobile m) { return m.Player; }

        private class InternalSorter : IComparer<Mobile>
        {
            private readonly Mobile m_From;
            public InternalSorter(Mobile from) { this.m_From = from; }
    ...
    public class TrackArrow : QuestArrow
    {
        ...
        public override void OnStop()
        {
            ...
            {
                Tracking.ClearTrackingInfo(this.m_From);
                this.m_From.SendLocalizedMessage(503177);
            }
        }
    }
    ...
    public class TrackTimer : Timer
    {
        ...
        protected override void OnTick()
        {
            ...
            else if (this.m_From.NetState == null || this.m_From.Deleted || this.m_Target.Deleted || this.m_From.Map != this.m_Target.Map || !this.m_From.InRange(this.m_Target, this.m_Range) ||
                (this.m_Target is Mobile && (((Mobile)this.m_Target).Hidden && ((Mobile)this.m_Target).AccessLevel > this.m_From.AccessLevel)))
            {
                this.m_Arrow.Stop();
                this.Stop();
                return;
