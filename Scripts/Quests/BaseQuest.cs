    ...
    { 
		...
		{ 
			...
			if (!resignChain)
			{
				try
				{
					if (Quester != null && !QuestHelper.FirstChainQuest(this, Quester))
						UpdateChain();
				}
				catch (Exception ex)
				{
					Console.WriteLine($"Error in OnResign: {ex.Message}");
				}
			}
			...
			if (m_Objectives != null)
			{
				for (int i = 0; i < m_Objectives.Count; i++)
				{ 
					if (m_Objectives[i] is ObtainObjective obtain)
					{
						if (m_Owner != null && obtain.Obtain != null)
							QuestHelper.RemoveStatus(m_Owner, obtain.Obtain);
					}
					else if (m_Objectives[i] is DeliverObjective deliver)
					{
						if (m_Owner != null && deliver.Delivery != null)
