    ...
    public abstract class Achievement
    {
        ...
        public virtual void AwardSovereigns(PlayerMobile pm)
        {
            if (AchievementSystem.SovereignMultiplier > 0.0)
            {
				...
				if (award > 0)
				{
					// Award sovereigns as usual
					pm.AccountSovereigns += award;
					pm.SendMessage("You have earned {0} account sovereigns!", award);

					// Also award MaxxiaScrolls: 1 per 5 sovereigns
					int scrollCount = award / 5;

					if (scrollCount > 0)
					{
						for (int i = 0; i < scrollCount; i++)
						{
							var scroll = new MaxxiaScroll();
							pm.AddToBackpack(scroll);
						}

						pm.SendMessage("You have been awarded {0} MaxxiaScroll{1} for completing this achievement!", scrollCount, scrollCount > 1 ? "s" : "");
					}
				}
            }
        }
