...
namespace Server.Engines.XmlSpawner2
{
    public class XmlTheftStrike : XmlAttachment
    {
        ...
        private void TheftItemFromTarget(Mobile attacker, Mobile defender)
        {
            ...
            // Get all items that are NOT blessed
            List<Item> stealableItems = backpack.Items
                .Where(item => !item.CheckBlessed(defender)) // Check if the item is blessed for this player
                .ToList();
            ...
            Item stolenItem = stealableItems[Utility.Random(stealableItems.Count)];
