...
{
    public class XmlSparkleBreath : XmlAttachment
    {
        ...
        public XmlSparkleBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSparkleBreath() { }

        [Attachable]
        public XmlSparkleBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x373A, 30, 10, 0, 0); // Use a suitable effect for the breath attack
