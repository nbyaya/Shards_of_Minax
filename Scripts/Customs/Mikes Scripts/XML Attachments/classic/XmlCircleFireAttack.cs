...
{
    public class XmlCircleFireAttack : XmlAttachment
    {
        ...
        public XmlCircleFireAttack(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlCircleFireAttack() { }

        [Attachable]
        public XmlCircleFireAttack(double refractory, int damage, int radius, int thickness)
        {
        ...
        private void DealDamageAtPoint(Point3D p, Map map, Mobile owner)
        {
            ...
            foreach (Mobile m in eable)
            {
                ...
                {
                    m.Damage(m_Damage, owner); // Pass the owner (Mobile) instead of this (XmlCircleFireAttack)
                }
            }
            eable.Free();
        }
