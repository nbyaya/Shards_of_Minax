...
{
    public class XmlCrankBreath : XmlAttachment
    {
        ...
        public XmlCrankBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlCrankBreath() { }

        [Attachable]
        public XmlCrankBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x1934, 30, 10, 0, 0); // Use a suitable effect for the breath attack
