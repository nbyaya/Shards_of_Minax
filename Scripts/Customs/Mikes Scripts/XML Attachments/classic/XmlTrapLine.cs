...
{
    public class XmlTrapLine : XmlAttachment
    {
        ...
        public XmlTrapLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlTrapLine() { }

        [Attachable]
        public XmlTrapLine(double refractory, int damage, int range)
        {
        ...
        public void PerformLineBreathAttack(Mobile owner)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                if (map.CanFit(targetX, targetY, start.Z, 16, false, false))
                {
                    ...
                    int flameHue = 0;
                    Effects.SendLocationEffect(p, map, 0x1D99, 16, 10, flameHue, 0);
