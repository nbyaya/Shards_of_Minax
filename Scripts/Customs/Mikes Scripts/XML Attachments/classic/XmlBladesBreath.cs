...
{
    public class XmlBladesBreath : XmlAttachment
    {
        ...
        public XmlBladesBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlBladesBreath() { }

        [Attachable]
        public XmlBladesBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x37A0, 30, 10, 0, 0); // Use a suitable effect for the breath attack
