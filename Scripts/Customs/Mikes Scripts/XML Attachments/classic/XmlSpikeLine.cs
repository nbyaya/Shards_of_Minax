...
{
    public class XmlSpikeLine : XmlAttachment
    {
        ...
        public XmlSpikeLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSpikeLine() { }

        [Attachable]
        public XmlSpikeLine(double refractory, int damage, int range)
        {
        ...
        public void PerformLineBreathAttack(Mobile owner)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                if (map.CanFit(targetX, targetY, start.Z, 16, false, false))
                {
                    ...
                    int flameHue = 0;
                    Effects.SendLocationEffect(p, map, 0x11A0, 16, 10, flameHue, 0);
