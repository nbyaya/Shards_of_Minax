...
{
    public class XmlNutcrackerBreath : XmlAttachment
    {
        ...
        public XmlNutcrackerBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlNutcrackerBreath() { }

        [Attachable]
        public XmlNutcrackerBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x46F5, 30, 10, 0, 0); // Use a suitable effect for the breath attack
