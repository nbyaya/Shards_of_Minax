...
{
    public class XmlVortexBreath : XmlAttachment
    {
        ...
        public XmlVortexBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlVortexBreath() { }

        [Attachable]
        public XmlVortexBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x3789, 30, 10, 0, 0); // Use a suitable effect for the breath attack
