...
{
    public class XmlAxeBreath : XmlAttachment
    {
        ...
        public XmlAxeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlAxeBreath() { }

        [Attachable]
        public XmlAxeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x1193, 30, 10, 0, 0); // Use a suitable effect for the breath attack
