...
{
    public class XmlGlowBreath : XmlAttachment
    {
        ...
        public XmlGlowBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlGlowBreath() { }

        [Attachable]
        public XmlGlowBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x37B9, 30, 10, 0, 0); // Use a suitable effect for the breath attack
