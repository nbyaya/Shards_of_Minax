...
{
    public class XmlHeartBreath : XmlAttachment
    {
        ...
        public XmlHeartBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlHeartBreath() { }

        [Attachable]
        public XmlHeartBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x49D0, 30, 10, 0, 0); // Use a suitable effect for the breath attack
