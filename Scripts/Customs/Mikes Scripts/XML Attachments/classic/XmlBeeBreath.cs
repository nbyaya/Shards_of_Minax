...
{
    public class XmlBeeBreath : XmlAttachment
    {
        ...
        public XmlBeeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlBeeBreath() { }

        [Attachable]
        public XmlBeeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x0921B, 30, 10, 0, 0); // Use a suitable effect for the breath attack
