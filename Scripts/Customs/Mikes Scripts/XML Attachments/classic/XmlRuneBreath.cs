...
{
    public class XmlRuneBreath : XmlAttachment
    {
        ...
        public XmlRuneBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlRuneBreath() { }

        [Attachable]
        public XmlRuneBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x0E62, 30, 10, 0, 0); // Use a suitable effect for the breath attack
