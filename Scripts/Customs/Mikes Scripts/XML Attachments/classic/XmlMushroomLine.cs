...
{
    public class XmlMushroomLine : XmlAttachment
    {
        ...
        public XmlMushroomLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlMushroomLine() { }

        [Attachable]
        public XmlMushroomLine(double refractory, int damage, int range)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int mushroomHue = 0x8A;  // Adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x0DB0, 16, 10, mushroomHue, 0);  // Adjust effect ID if needed
