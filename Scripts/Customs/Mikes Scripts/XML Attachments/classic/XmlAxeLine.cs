...
{
    public class XmlAxeLine : XmlAttachment
    {
        ...
        public XmlAxeLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlAxeLine() { }

        [Attachable]
        public XmlAxeLine(double refractory, int damage, int range)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int effectHue = 0; // Adjust effect hue if needed
                        Effects.SendLocationEffect(p, map, 0x1193, 16, 10, effectHue, 0);
