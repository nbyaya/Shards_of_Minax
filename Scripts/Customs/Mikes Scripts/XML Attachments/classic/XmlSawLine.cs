...
{
    public class XmlSawLine : XmlAttachment
    {
        ...
        public XmlSawLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSawLine() { }

        [Attachable]
        public XmlSawLine(double refractory, int damage, int range)
        {
        ...
        {
            return String.Format("Saw Line Attack: {0} damage over {1} tiles every {2} seconds.", m_Damage, m_Range, m_Refractory.TotalSeconds);
        }
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int effectId = 0x11B5;  // Or adjust effect ID if needed
                        Effects.SendLocationEffect(p, map, effectId, 16, 10, 0, 0);
