...
{
    public class XmlCurtainCircle : XmlAttachment
    {
        ...
        public XmlCurtainCircle(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlCurtainCircle() { }

        [Attachable]
        public XmlCurtainCircle(double refractory, int damage, int radius, int thickness)
        {
            ...
            for (int i = -m_Radius; i <= m_Radius; i++)
            {
                for (int j = -m_Radius; j <= m_Radius; j++)
                {
                    if (i * i + j * j <= m_Radius * m_Radius && i * i + j * j >= (m_Radius - m_Thickness) * (m_Radius - m_Thickness))
                    {
                        ...
                        {
                            Effects.SendLocationEffect(p, map, 0x12E9, 16, 10, 0, 0); // Flamestrike animation with default hue
        ...
        private void DealDamageAtPoint(Point3D p, Map map, Mobile owner)
        {
            ...
            foreach (Mobile m in eable)
            {
                ...
                {
                    m.Damage(m_Damage, owner); // Pass the owner (Mobile) instead of this (XmlCurtainCircle)
                }
            }
            eable.Free();
        }
