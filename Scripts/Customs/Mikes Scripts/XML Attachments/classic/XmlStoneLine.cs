...
{
    public class XmlStoneLine : XmlAttachment
    {
        ...
        public XmlStoneLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlStoneLine() { }

        [Attachable]
        public XmlStoneLine(double refractory, int damage, int range)
        {
        ...
        public void PerformLineBreathAttack(Mobile owner)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                if (map.CanFit(targetX, targetY, start.Z, 16, false, false))
                {
                    ...
                    int flameHue = 0;
                    Effects.SendLocationEffect(p, map, 0x11B6, 16, 10, flameHue, 0);
