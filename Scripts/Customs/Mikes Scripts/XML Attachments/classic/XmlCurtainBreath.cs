...
{
    public class XmlCurtainBreath : XmlAttachment
    {
        ...
        public XmlCurtainBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlCurtainBreath() { }

        [Attachable]
        public XmlCurtainBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x12E9, 30, 10, 0, 0); // Use a suitable effect for the breath attack
