...
{
    public class XmlFTreeBreath : XmlAttachment
    {
        ...
        public XmlFTreeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlFTreeBreath() { }

        [Attachable]
        public XmlFTreeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x4A8E, 30, 10, 0, 0); // Use a suitable effect for the breath attack
