...
{
    public class XmlGasLine : XmlAttachment
    {
        ...
        public XmlGasLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlGasLine() { }

        [Attachable]
        public XmlGasLine(double refractory, int damage, int range)
        {
        ...
        {
            return String.Format("Gas Line Attack: {0} damage over {1} tiles every {2} seconds.", m_Damage, m_Range, m_Refractory.TotalSeconds);
        }
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int gasHue = 0;  // Adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x11A6, 16, 10, gasHue, 0);
