...
{
    public class XmlSmokeBreath : XmlAttachment
    {
        ...
        public XmlSmokeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSmokeBreath() { }

        [Attachable]
        public XmlSmokeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x3728, 30, 10, 0, 0); // Use a suitable effect for the breath attack
