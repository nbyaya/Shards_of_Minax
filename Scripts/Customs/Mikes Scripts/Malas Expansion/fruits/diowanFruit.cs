...
{
    public class diowanFruit : Food
    {
        [Constructable]
        public diowanFruit() : this(1)
        {
        }

        [Constructable]
        public diowanFruit(int amount) : base(amount, 0x0F7A) // Example fruit graphic
        {
            Weight = 1.0;
            FillFactor = 1;
            Name = "diowan fruit";
            Hue = 2422; // Same hue as the plant
        }

        public diowanFruit(Serial serial) : base(serial)
        {
        }
        ...
        public override bool Eat(Mobile from)
        {
            ...
            // 10% chance to give a diowanFruitSeed
            if (Utility.RandomDouble() < 0.1) // 0.1 = 10% chance
            {
                Item seed = new diowanFruitSeed();
                if (from.Backpack != null && from.Backpack.TryDropItem(from, seed, false))
                {
                    from.SendMessage("You find a diowan fruit seed as you eat the fruit.");
                }
                else
                {
                    seed.Delete();
    ...
    public class diowanFruitplant : Item
    {
        public static List<diowanFruitplant> plantes = new List<diowanFruitplant>();
        public bool IsHarvestable;

        [Constructable]
        public diowanFruitplant() : base(0x0C45) // Seeds graphic by default
        {
            Movable = false;
            Name = "a diowan fruit plant";
            Hue = 2422; // Constant hue
            IsHarvestable = false;
        ...
        private static void OnWorldSave(WorldSaveEventArgs e)
        {
            ...
            foreach (diowanFruitplant plant in plantes)
            {
                if (!plant.Deleted && !plant.IsHarvestable) // Only make unharvestable plantes harvestable
                {
                    if (rnd.Next(2) == 0) // 50% chance to become harvestable
                    {
                        ...
                        plant.ItemID = 0x0C8D; // plant graphic
                    }

                    plant.Hue = 2422; // Ensure hue is always 699
                }
            }
        }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (IsHarvestable)
			{
				...
				{
					Item diowanFruit = new diowanFruit();
					if (from.Backpack != null && from.Backpack.TryDropItem(from, diowanFruit, false))
					{
						if (i == 0) // Only display the harvest message once
						{
							from.SendMessage($"You harvest {totalFruits} diowan fruit{(totalFruits > 1 ? "s" : "")}.");
						}
					}
					else
					{
			...
			Hue = 2422; // Ensure hue remains 699
		}


        public diowanFruitplant(Serial serial) : base(serial)
        {
        }
    ...
    public class diowanFruitSeed : Item
    {
        [Constructable]
        public diowanFruitSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a diowan fruit seed";
			Hue = 2422; // Same hue as the plant
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                diowanFruitplant plant = new diowanFruitplant();
        ...
        public diowanFruitSeed(Serial serial) : base(serial)
        {
        }
