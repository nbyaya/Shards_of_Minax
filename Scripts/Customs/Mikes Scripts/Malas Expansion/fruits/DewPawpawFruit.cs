...
{
    public class DewPawpawFruit : Food
    {
        [Constructable]
        public DewPawpawFruit() : this(1)
        {
        }

        [Constructable]
        public DewPawpawFruit(int amount) : base(amount, 0x0C6D) // Example fruit graphic
        {
            Weight = 1.0;
            FillFactor = 1;
            Name = "DewPawpaw fruit";
            Hue = 527; // Same hue as the plant
        }

        public DewPawpawFruit(Serial serial) : base(serial)
        {
        }
        ...
        public override bool Eat(Mobile from)
        {
            ...
            // 10% chance to give a DewPawpawFruitSeed
            if (Utility.RandomDouble() < 0.1) // 0.1 = 10% chance
            {
                Item seed = new DewPawpawFruitSeed();
                if (from.Backpack != null && from.Backpack.TryDropItem(from, seed, false))
                {
                    from.SendMessage("You find a DewPawpaw fruit seed as you eat the fruit.");
                }
                else
                {
                    seed.Delete();
    ...
    public class DewPawpawFruitplant : Item
    {
        public static List<DewPawpawFruitplant> plantes = new List<DewPawpawFruitplant>();
        public bool IsHarvestable;

        [Constructable]
        public DewPawpawFruitplant() : base(0x0C45) // Seeds graphic by default
        {
            Movable = false;
            Name = "a DewPawpaw fruit plant";
            Hue = 527; // Constant hue
            IsHarvestable = false;
        ...
        private static void OnWorldSave(WorldSaveEventArgs e)
        {
            ...
            foreach (DewPawpawFruitplant plant in plantes)
            {
                if (!plant.Deleted && !plant.IsHarvestable) // Only make unharvestable plantes harvestable
                {
                    if (rnd.Next(2) == 0) // 50% chance to become harvestable
                    {
                        ...
                        plant.ItemID = 0x0CC7; // plant graphic
                    }

                    plant.Hue = 527; // Ensure hue is always 699
                }
            }
        }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (IsHarvestable)
			{
				...
				{
					Item DewPawpawFruit = new DewPawpawFruit();
					if (from.Backpack != null && from.Backpack.TryDropItem(from, DewPawpawFruit, false))
					{
						if (i == 0) // Only display the harvest message once
						{
							from.SendMessage($"You harvest {totalFruits} DewPawpaw fruit{(totalFruits > 1 ? "s" : "")}.");
						}
					}
					else
					{
						DewPawpawFruit.Delete();
						from.SendLocalizedMessage(500720); // You don't have enough room in your backpack!
						break;
					}
				}
			...
			Hue = 527; // Ensure hue remains 699
		}


        public DewPawpawFruitplant(Serial serial) : base(serial)
        {
        }
    ...
    public class DewPawpawFruitSeed : Item
    {
        [Constructable]
        public DewPawpawFruitSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a DewPawpaw fruit seed";
			Hue = 527; // Same hue as the plant
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                DewPawpawFruitplant plant = new DewPawpawFruitplant();
        ...
        public DewPawpawFruitSeed(Serial serial) : base(serial)
        {
        }
