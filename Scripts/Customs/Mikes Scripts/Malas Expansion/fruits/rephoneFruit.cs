...
{
    public class rephoneFruit : Food
    {
        [Constructable]
        public rephoneFruit() : this(1)
        {
        }

        [Constructable]
        public rephoneFruit(int amount) : base(amount, 0x0C70) // Example fruit graphic
        {
            Weight = 1.0;
            FillFactor = 1;
            Name = "rephone fruit";
            Hue = 6; // Same hue as the plant
        }

        public rephoneFruit(Serial serial) : base(serial)
        {
        }
        ...
        public override bool Eat(Mobile from)
        {
            ...
            // 10% chance to give a rephoneFruitSeed
            if (Utility.RandomDouble() < 0.1) // 0.1 = 10% chance
            {
                Item seed = new rephoneFruitSeed();
                if (from.Backpack != null && from.Backpack.TryDropItem(from, seed, false))
                {
                    from.SendMessage("You find a rephone fruit seed as you eat the fruit.");
                }
                else
                {
                    seed.Delete();
    ...
    public class rephoneFruitplant : Item
    {
        public static List<rephoneFruitplant> plantes = new List<rephoneFruitplant>();
        public bool IsHarvestable;

        [Constructable]
        public rephoneFruitplant() : base(0x0C45) // Seeds graphic by default
        {
            Movable = false;
            Name = "a rephone fruit plant";
            Hue = 6; // Constant hue
            IsHarvestable = false;
        ...
        private static void OnWorldSave(WorldSaveEventArgs e)
        {
            ...
            foreach (rephoneFruitplant plant in plantes)
            {
                if (!plant.Deleted && !plant.IsHarvestable) // Only make unharvestable plantes harvestable
                {
                    if (rnd.Next(2) == 0) // 50% chance to become harvestable
                    {
                        ...
                        plant.ItemID = 0x0D2C; // plant graphic
                    }

                    plant.Hue = 6; // Ensure hue is always 699
                }
            }
        }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (IsHarvestable)
			{
				...
				{
					Item rephoneFruit = new rephoneFruit();
					if (from.Backpack != null && from.Backpack.TryDropItem(from, rephoneFruit, false))
					{
						if (i == 0) // Only display the harvest message once
						{
							from.SendMessage($"You harvest {totalFruits} rephone fruit{(totalFruits > 1 ? "s" : "")}.");
						}
					}
					else
					{
			...
			Hue = 6; // Ensure hue remains 699
		}


        public rephoneFruitplant(Serial serial) : base(serial)
        {
        }
    ...
    public class rephoneFruitSeed : Item
    {
        [Constructable]
        public rephoneFruitSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a rephone fruit seed";
			Hue = 6; // Same hue as the plant
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                rephoneFruitplant plant = new rephoneFruitplant();
        ...
        public rephoneFruitSeed(Serial serial) : base(serial)
        {
        }
