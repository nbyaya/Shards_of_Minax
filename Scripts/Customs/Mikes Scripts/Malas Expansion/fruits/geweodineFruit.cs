...
{
    public class geweodineFruit : Food
    {
        [Constructable]
        public geweodineFruit() : this(1)
        {
        }

        [Constructable]
        public geweodineFruit(int amount) : base(amount, 0x0F7C) // Example fruit graphic
        {
            Weight = 1.0;
            FillFactor = 1;
            Name = "geweodine fruit";
            Hue = 2048; // Same hue as the plant
        }

        public geweodineFruit(Serial serial) : base(serial)
        {
        }
        ...
        public override bool Eat(Mobile from)
        {
            ...
            // 10% chance to give a geweodineFruitSeed
            if (Utility.RandomDouble() < 0.1) // 0.1 = 10% chance
            {
                Item seed = new geweodineFruitSeed();
                if (from.Backpack != null && from.Backpack.TryDropItem(from, seed, false))
                {
                    from.SendMessage("You find a geweodine fruit seed as you eat the fruit.");
                }
                else
                {
                    seed.Delete();
    ...
    public class geweodineFruitplant : Item
    {
        public static List<geweodineFruitplant> plantes = new List<geweodineFruitplant>();
        public bool IsHarvestable;

        [Constructable]
        public geweodineFruitplant() : base(0x0C45) // Seeds graphic by default
        {
            Movable = false;
            Name = "a geweodine fruit plant";
            Hue = 2048; // Constant hue
            IsHarvestable = false;
        ...
        private static void OnWorldSave(WorldSaveEventArgs e)
        {
            ...
            foreach (geweodineFruitplant plant in plantes)
            {
                if (!plant.Deleted && !plant.IsHarvestable) // Only make unharvestable plantes harvestable
                {
                    if (rnd.Next(2) == 0) // 50% chance to become harvestable
                    {
                        ...
                        plant.ItemID = 0x0CA1; // plant graphic
                    }

                    plant.Hue = 2048; // Ensure hue is always 699
                }
            }
        }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (IsHarvestable)
			{
				...
				{
					Item geweodineFruit = new geweodineFruit();
					if (from.Backpack != null && from.Backpack.TryDropItem(from, geweodineFruit, false))
					{
						if (i == 0) // Only display the harvest message once
						{
							from.SendMessage($"You harvest {totalFruits} geweodine fruit{(totalFruits > 1 ? "s" : "")}.");
						}
					}
					else
					{
						geweodineFruit.Delete();
						from.SendLocalizedMessage(500720); // You don't have enough room in your backpack!
						break;
					}
				}
			...
			Hue = 2048; // Ensure hue remains 699
		}


        public geweodineFruitplant(Serial serial) : base(serial)
        {
        }
    ...
    public class geweodineFruitSeed : Item
    {
        [Constructable]
        public geweodineFruitSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a geweodine fruit seed";
			Hue = 2048; // Same hue as the plant
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                geweodineFruitplant plant = new geweodineFruitplant();
        ...
        public geweodineFruitSeed(Serial serial) : base(serial)
        {
        }
