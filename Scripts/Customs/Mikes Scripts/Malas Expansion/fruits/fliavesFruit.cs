...
{
    public class fliavesFruit : Food
    {
        [Constructable]
        public fliavesFruit() : this(1)
        {
        }

        [Constructable]
        public fliavesFruit(int amount) : base(amount, 0x1722) // Example fruit graphic
        {
            Weight = 1.0;
            FillFactor = 1;
            Name = "fliaves fruit";
            Hue = 2216; // Same hue as the plant
        }

        public fliavesFruit(Serial serial) : base(serial)
        {
        }
        ...
        public override bool Eat(Mobile from)
        {
            ...
            // 10% chance to give a fliavesFruitSeed
            if (Utility.RandomDouble() < 0.1) // 0.1 = 10% chance
            {
                Item seed = new fliavesFruitSeed();
                if (from.Backpack != null && from.Backpack.TryDropItem(from, seed, false))
                {
                    from.SendMessage("You find a fliaves fruit seed as you eat the fruit.");
                }
                else
                {
                    seed.Delete();
    ...
    public class fliavesFruitplant : Item
    {
        public static List<fliavesFruitplant> plantes = new List<fliavesFruitplant>();
        public bool IsHarvestable;

        [Constructable]
        public fliavesFruitplant() : base(0x0C45) // Seeds graphic by default
        {
            Movable = false;
            Name = "a fliaves fruit plant";
            Hue = 2216; // Constant hue
            IsHarvestable = false;
        ...
        private static void OnWorldSave(WorldSaveEventArgs e)
        {
            ...
            foreach (fliavesFruitplant plant in plantes)
            {
                if (!plant.Deleted && !plant.IsHarvestable) // Only make unharvestable plantes harvestable
                {
                    if (rnd.Next(2) == 0) // 50% chance to become harvestable
                    {
                        ...
                        plant.ItemID = 0x0D27; // plant graphic
                    }

                    plant.Hue = 2216; // Ensure hue is always 699
                }
            }
        }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (IsHarvestable)
			{
				...
				{
					Item fliavesFruit = new fliavesFruit();
					if (from.Backpack != null && from.Backpack.TryDropItem(from, fliavesFruit, false))
					{
						if (i == 0) // Only display the harvest message once
						{
							from.SendMessage($"You harvest {totalFruits} fliaves fruit{(totalFruits > 1 ? "s" : "")}.");
						}
					}
					else
					{
			...
			Hue = 2216; // Ensure hue remains 699
		}


        public fliavesFruitplant(Serial serial) : base(serial)
        {
        }
    ...
    public class fliavesFruitSeed : Item
    {
        [Constructable]
        public fliavesFruitSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a fliaves fruit seed";
			Hue = 2216; // Same hue as the plant
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                fliavesFruitplant plant = new fliavesFruitplant();
        ...
        public fliavesFruitSeed(Serial serial) : base(serial)
        {
        }
