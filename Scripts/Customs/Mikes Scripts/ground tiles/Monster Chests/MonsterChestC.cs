...
{
    public class MonsterChestC : WoodenChest
    {
        private bool m_Activated = false;

        [Constructable]
        public MonsterChestC() : base()
        {
				this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int orcCount = Utility.Random(1, 6);
            int orcbomberCount = Utility.Random(1, 6);
            int orccaptainCount = Utility.Random(1, 3);

            for (int i = 0; i < orcCount; i++)
                monsters.Add(new Orc());

            for (int i = 0; i < orcbomberCount; i++)
                monsters.Add(new OrcBomber());

            for (int i = 0; i < orccaptainCount; i++)
                monsters.Add(new OrcCaptain());
        ...
        public MonsterChestC(Serial serial) : base(serial)
        {
        }
