...
{
    public class MonsterChestD : WoodenChest
    {
        private bool m_Activated = false;

        [Constructable]
        public MonsterChestD() : base()
        {
				this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int orcchopperCount = Utility.Random(1, 6);
            int orcbruteCount = Utility.Random(1, 6);
            int orcishmageCount = Utility.Random(1, 3);

            for (int i = 0; i < orcchopperCount; i++)
                monsters.Add(new OrcChopper());

            for (int i = 0; i < orcbruteCount; i++)
                monsters.Add(new OrcBrute());

            for (int i = 0; i < orcishmageCount; i++)
                monsters.Add(new OrcishMage());
        ...
        public MonsterChestD(Serial serial) : base(serial)
        {
        }
