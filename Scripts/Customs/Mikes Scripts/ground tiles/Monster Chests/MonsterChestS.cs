...
{
    public class MonsterChestS : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestS() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int evilClownCount = Utility.Random(1, 6);

            for (int i = 0; i < evilClownCount; i++)
                monsters.Add(new EvilClown());
        ...
        public MonsterChestS(Serial serial) : base(serial)
        {
        }
