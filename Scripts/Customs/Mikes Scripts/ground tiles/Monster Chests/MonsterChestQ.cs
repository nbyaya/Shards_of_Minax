...
{
    public class MonsterChestQ : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestQ() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int bardCount = Utility.Random(1, 6);

            for (int i = 0; i < bardCount; i++)
                monsters.Add(new Bard()); // Spawn Bard creatures
        ...
        public MonsterChestQ(Serial serial) : base(serial)
        {
        }
