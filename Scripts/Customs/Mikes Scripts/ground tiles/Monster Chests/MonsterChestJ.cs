...
{
    public class MonsterChestJ : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestJ() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int princessCount = Utility.Random(1, 6); // You can adjust the count as needed

            for (int i = 0; i < princessCount; i++)
                monsters.Add(new HostilePrincess()); // Assuming HostilePrincess is a custom creature class
        ...
        public MonsterChestJ(Serial serial) : base(serial)
        {
        }
