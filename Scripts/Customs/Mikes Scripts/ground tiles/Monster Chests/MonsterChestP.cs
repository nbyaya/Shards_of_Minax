...
{
    public class MonsterChestP : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestP() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int cannibalCount = Utility.Random(1, 6);

            for (int i = 0; i < cannibalCount; i++)
                monsters.Add(new Cannibal());
        ...
        public MonsterChestP(Serial serial) : base(serial)
        {
        }
