...
{
    public class MonsterChestF : WoodenChest
    {
        private bool m_Activated = false;

        [Constructable]
        public MonsterChestF() : base()
        {
				this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int daemonCount = Utility.Random(1, 3);
            int drakeCount = Utility.Random(1, 3);
            int ettinCount = Utility.Random(1, 3);

            for (int i = 0; i < daemonCount; i++)
                monsters.Add(new Daemon());

            for (int i = 0; i < drakeCount; i++)
                monsters.Add(new Drake());

            for (int i = 0; i < ettinCount; i++)
                monsters.Add(new Ettin());
        ...
        public MonsterChestF(Serial serial) : base(serial)
        {
        }
