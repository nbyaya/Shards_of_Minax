...
{
    public class MonsterChestT : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestT() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int bountyHunterCount = Utility.Random(1, 6);

            for (int i = 0; i < bountyHunterCount; i++)
                monsters.Add(new BountyHunter()); // Make sure to create the BountyHunter class
        ...
        public MonsterChestT(Serial serial) : base(serial)
        {
        }
