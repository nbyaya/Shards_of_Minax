...
{
    public class MonsterChestZ : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestZ() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int officerCount = Utility.Random(1, 6);

            for (int i = 0; i < officerCount; i++)
                monsters.Add(new StarfleetOfficer()); // Make sure StarfleetOfficer class exists in your code.
        ...
        public MonsterChestZ(Serial serial) : base(serial)
        {
        }
