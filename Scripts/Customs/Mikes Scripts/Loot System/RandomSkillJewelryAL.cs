...
public class RandomSkillJewelryAL : BaseJewel
{
    private static string[] prefixes = { "Archer's", "Fletcher's" };
    ...
    [Constructable]
    public RandomSkillJewelryAL() : base(GetRandomItemID(), Layer.Ring)
    {
    ...
    private void InitializeJewelryAttributes(BaseJewel tempJewelry)
    {
        ...
        if (tierChance < 0.05) // Very rare
        {
            ...
            SkillBonuses.SetValues(0, SkillName.Archery, Utility.RandomMinMax(0, 100));
            SkillBonuses.SetValues(1, SkillName.Fletching, Utility.RandomMinMax(0, 100));
            SkillBonuses.SetValues(2, SkillName.Tactics, Utility.RandomMinMax(0, 100));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 4);
        }
        else if (tierChance < 0.2) // Rare
        {
            this.Attributes.Luck = rand.Next(40, 79);
            SkillBonuses.SetValues(0, SkillName.Archery, Utility.RandomMinMax(0, 35));
            SkillBonuses.SetValues(1, SkillName.Fletching, Utility.RandomMinMax(0, 35));
            SkillBonuses.SetValues(2, SkillName.Tactics, Utility.RandomMinMax(0, 35));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 3);
        }
        else if (tierChance < 0.5) // Uncommon
        {
            this.Attributes.Luck = rand.Next(20, 39);
            SkillBonuses.SetValues(0, SkillName.Archery, Utility.RandomMinMax(0, 24));
            SkillBonuses.SetValues(1, SkillName.Fletching, Utility.RandomMinMax(0, 24));
            SkillBonuses.SetValues(2, SkillName.Tactics, Utility.RandomMinMax(0, 24));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 2);
        }
        else // Common
        {
            this.Attributes.Luck = rand.Next(1, 19);
            SkillBonuses.SetValues(0, SkillName.Archery, Utility.RandomMinMax(0, 10));
            SkillBonuses.SetValues(1, SkillName.Fletching, Utility.RandomMinMax(0, 10));
            SkillBonuses.SetValues(2, SkillName.Tactics, Utility.RandomMinMax(0, 10));
        }
    }
    ...
    public RandomSkillJewelryAL(Serial serial) : base(serial) { }
