...
public class RandomSkillJewelryAC : BaseJewel
{
    private static string[] prefixes = { "Sneaky", "Shadowy" };
    ...
    [Constructable]
    public RandomSkillJewelryAC() : base(GetRandomItemID(), Layer.Ring)
    {
    ...
    private void InitializeJewelryAttributes(BaseJewel tempJewelry)
    {
        ...
        if (tierChance < 0.05) // Very rare
        {
            ...
            SkillBonuses.SetValues(0, SkillName.Snooping, Utility.RandomMinMax(0, 100));
            SkillBonuses.SetValues(1, SkillName.Stealing, Utility.RandomMinMax(0, 100));
            SkillBonuses.SetValues(2, SkillName.Hiding, Utility.RandomMinMax(0, 100));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 4);
        }
        else if (tierChance < 0.2) // Rare
        {
            this.Attributes.Luck = rand.Next(40, 79);
            SkillBonuses.SetValues(0, SkillName.Snooping, Utility.RandomMinMax(0, 35));
            SkillBonuses.SetValues(1, SkillName.Stealing, Utility.RandomMinMax(0, 35));
            SkillBonuses.SetValues(2, SkillName.Hiding, Utility.RandomMinMax(0, 35));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 3);
        }
        else if (tierChance < 0.5) // Uncommon
        {
            this.Attributes.Luck = rand.Next(20, 39);
            SkillBonuses.SetValues(0, SkillName.Snooping, Utility.RandomMinMax(0, 24));
            SkillBonuses.SetValues(1, SkillName.Stealing, Utility.RandomMinMax(0, 24));
            SkillBonuses.SetValues(2, SkillName.Hiding, Utility.RandomMinMax(0, 24));
            this.Attributes.CastSpeed = Utility.RandomMinMax(1, 2);
        }
        else // Common
        {
            this.Attributes.Luck = rand.Next(1, 19);
            SkillBonuses.SetValues(0, SkillName.Snooping, Utility.RandomMinMax(0, 10));
            SkillBonuses.SetValues(1, SkillName.Stealing, Utility.RandomMinMax(0, 10));
            SkillBonuses.SetValues(2, SkillName.Hiding, Utility.RandomMinMax(0, 10));
        }
    }
    ...
    public RandomSkillJewelryAC(Serial serial) : base(serial) { }
