...
{
    public class MinorPoisonBomb : Item
    {
        ...
        public MinorPoisonBomb(Serial serial) : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
			...
			if (from.Skills[SkillName.Poisoning].Base < 50.0)
            {
                from.SendMessage("You need at least 50 Poisoning skill to use this bomb.");
                return;
            }
        ...
        {
            private readonly MinorPoisonBomb m_MinorPoisonBomb;

            public ThrowTarget(MinorPoisonBomb MinorPoisonBomb) : base(12, true, TargetFlags.None)
            {
                m_MinorPoisonBomb = MinorPoisonBomb;
            }

            protected override void OnTarget(Mobile from, object targeted)
            {
                if (m_MinorPoisonBomb.Deleted || m_MinorPoisonBomb.Map == Map.Internal)
                    return;
                ...
                Effects.SendMovingEffect(from, to, m_MinorPoisonBomb.ItemID, 7, 0, false, false, m_MinorPoisonBomb.Hue, 0);

                // Remove the bomb from the player's inventory
                m_MinorPoisonBomb.MoveToWorld(new Point3D(p), map);

                // Cancel any existing timer
                if (m_MinorPoisonBomb.m_Timer != null)
                {
                    m_MinorPoisonBomb.m_Timer.Stop();
                    m_MinorPoisonBomb.m_Timer = null;
                }

                // Set a new timer for the explosion at the target location
                Timer.DelayCall(TimeSpan.FromSeconds(1.0), () => m_MinorPoisonBomb.Explode(from, new Point3D(p), map));
            }
        }
    }
}