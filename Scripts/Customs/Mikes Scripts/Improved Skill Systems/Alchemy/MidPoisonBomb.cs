...
namespace Server.Items
{
    public class PoisonBomb : Item
    {
        ...
        public PoisonBomb(Serial serial) : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
			...
			if (from.Skills[SkillName.Poisoning].Base < 75.0)
            {
                from.SendMessage("You need at least 75 Poisoning skill to use this bomb.");
                return;
            }
        ...
        {
            private readonly PoisonBomb m_PoisonBomb;

            public ThrowTarget(PoisonBomb poisonBomb) : base(12, true, TargetFlags.None)
            {
                m_PoisonBomb = poisonBomb;
            }

            protected override void OnTarget(Mobile from, object targeted)
            {
                if (m_PoisonBomb.Deleted || m_PoisonBomb.Map == Map.Internal)
                    return;
                ...
                Effects.SendMovingEffect(from, to, m_PoisonBomb.ItemID, 7, 0, false, false, m_PoisonBomb.Hue, 0);

                // Remove the bomb from the player's inventory
                m_PoisonBomb.MoveToWorld(new Point3D(p), map);

                // Cancel any existing timer
                if (m_PoisonBomb.m_Timer != null)
                {
                    m_PoisonBomb.m_Timer.Stop();
                    m_PoisonBomb.m_Timer = null;
                }

                // Set a new timer for the explosion at the target location
                Timer.DelayCall(TimeSpan.FromSeconds(1.0), () => m_PoisonBomb.Explode(from, new Point3D(p), map));
            }
        }
    }
}