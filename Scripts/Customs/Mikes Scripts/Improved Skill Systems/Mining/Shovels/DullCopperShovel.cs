...
{
    public class DullCopperShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a dull copper shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public DullCopperShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x42D;
        ...
        public DullCopperShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 135)
            {
                from.SendMessage("You need at least 135 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new DullCopperOre());
                    m_Uses--;

                    if (m_Uses <= 0)
                    {
                        from.SendMessage("The golden shovel breaks!");
                        this.Delete();
                    }
                }
                else
