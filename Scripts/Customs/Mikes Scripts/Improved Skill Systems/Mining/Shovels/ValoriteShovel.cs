...
{
    public class ValoriteShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a valorite shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public ValoriteShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x43A;
        ...
        public ValoriteShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 190)
            {
                from.SendMessage("You need at least 190 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new ValoriteOre());
                    m_Uses--;
