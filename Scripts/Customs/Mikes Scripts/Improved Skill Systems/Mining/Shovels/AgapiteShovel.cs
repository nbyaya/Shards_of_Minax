...
{
    public class AgapiteShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a agapite shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public AgapiteShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x435;
        ...
        public AgapiteShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 170)
            {
                from.SendMessage("You need at least 170 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new AgapiteOre());
                    m_Uses--;

                    if (m_Uses <= 0)
                    {
                        from.SendMessage("The agapite shovel breaks!");
                        this.Delete();
                    }
                }
                else
