...
namespace Server.Items
{
    public class GoldenShovel : Item
    {
        ...
        {
            if (from.Skills.Mining.Value < 125)
            {
                from.SendMessage("You need at least 125 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new GoldOre());
                    m_Uses--;
