...
{
    public class BronzeShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a bronze shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public BronzeShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x434;
        ...
        public BronzeShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 160)
            {
                from.SendMessage("You need at least 160 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new BronzeOre());
                    m_Uses--;

                    if (m_Uses <= 0)
                    {
                        from.SendMessage("The bronze shovel breaks!");
                        this.Delete();
                    }
                }
                else
