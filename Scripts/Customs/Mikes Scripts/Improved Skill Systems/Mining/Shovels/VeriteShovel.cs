...
{
    public class VeriteShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a verite shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public VeriteShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x439;
        ...
        public VeriteShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 180)
            {
                from.SendMessage("You need at least 180 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new VeriteOre());
                    m_Uses--;
