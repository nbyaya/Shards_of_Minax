...
{
    public class YewAxe : Item
    {
        public override string DefaultName
        {
            get { return "a yew axe"; }
        }

        private int m_Uses;

        [Constructable]
        public YewAxe() : base(0xF43) // Axe item ID
        {
            Weight = 5.0;
            Hue = 0x42A; // Unique color for the axe
            m_Uses = 50; // Total uses
        }

        public YewAxe(Serial serial) : base(serial)
        {
        }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new YewLog()); // OakLog item to be created
                    m_Uses--;
