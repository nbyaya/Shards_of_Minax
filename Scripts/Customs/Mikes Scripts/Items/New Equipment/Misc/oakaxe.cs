...
{
    public class OakAxe : Item
    {
        public override string DefaultName
        {
            get { return "a oak axe"; }
        }

        private int m_Uses;

        [Constructable]
        public OakAxe() : base(0xF43) // Axe item ID
        {
            Weight = 5.0;
            Hue = 0x972; // Unique color for the axe
            m_Uses = 50; // Total uses
        }

        public OakAxe(Serial serial) : base(serial)
        {
        }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new OakLog()); // OakLog item to be created
                    m_Uses--;
