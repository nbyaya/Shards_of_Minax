...
{
    public class AshAxe : Item
    {
        public override string DefaultName
        {
            get { return "a ash axe"; }
        }

        private int m_Uses;

        [Constructable]
        public AshAxe() : base(0xF43) // Axe item ID
        {
            Weight = 5.0;
            Hue = 0x428; // Unique color for the axe
            m_Uses = 50; // Total uses
        }

        public AshAxe(Serial serial) : base(serial)
        {
        }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new AshLog()); // OakLog item to be created
                    m_Uses--;
