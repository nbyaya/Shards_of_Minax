...
{
    public class ShadowIronShovel : Item
    {
        
		public override string DefaultName
        {
            get { return "a shadow iron shovel"; }
        }

        private int m_Uses;

        [Constructable]
        public ShadowIronShovel() : base(0xF39)
        {
            Weight = 5.0;
            Hue = 0x42E;
        ...
        public ShadowIronShovel(Serial serial) : base(serial)
        {
        }
        ...
        {
            if (from.Skills.Mining.Value < 140)
            {
                from.SendMessage("You need at least 140 mining skill to use this.");
                return;
            }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new ShadowIronOre());
                    m_Uses--;

                    if (m_Uses <= 0)
                    {
                        from.SendMessage("The shadow iron shovel breaks!");
                        this.Delete();
                    }
                }
                else
