using System;
using Server;
using Server.Items;
using Server.Engines.XmlSpawner2;

public class ShadowMastersTreads : LeatherGloves
{
    [Constructable]
    public ShadowMastersTreads()
    {
        Name = "Shadow Master's Treads";
        Hue = Utility.Random(1601, 1700);
        BaseArmorRating = Utility.RandomMinMax(60, 80);
        ArmorAttributes.LowerStatReq = 50;
        Attributes.BonusDex = 25;
        Attributes.BonusInt = 10;
        Attributes.RegenStam = 10;
        SkillBonuses.SetValues(0, SkillName.Hiding, 50.0);
        SkillBonuses.SetValues(1, SkillName.Stealth, 50.0);
        SkillBonuses.SetValues(2, SkillName.Ninjitsu, 50.0);
        PhysicalBonus = 20;
        ColdBonus = 20;
        FireBonus = 10;
        EnergyBonus = 30;
        PoisonBonus = 20;
        XmlAttach.AttachTo(this, new XmlLevelItem());
    }

    public ShadowMastersTreads(Serial serial) : base(serial)
    {
    }

    public override void Serialize(GenericWriter writer)
    {
        base.Serialize(writer);
        writer.Write((int)0); // version
    }

    public override void Deserialize(GenericReader reader)
    {
        base.Deserialize(reader);
        int version = reader.ReadInt();
    }
}
