using System;
using Server;
using Server.Items;
using Server.Engines.XmlSpawner2;

public class BeastsWhisperersRobe : LeatherChest
{
    [Constructable]
    public BeastsWhisperersRobe()
    {
        Name = "Beast Whisperer's Robe";
        Hue = Utility.Random(1, 1000);
        BaseArmorRating = Utility.RandomMinMax(20, 60);
        AbsorptionAttributes.EaterKinetic = 25;
        ArmorAttributes.LowerStatReq = 15;
        Attributes.BonusInt = 35;
        Attributes.RegenMana = 5;
        SkillBonuses.SetValues(0, SkillName.AnimalTaming, 25.0);
        SkillBonuses.SetValues(1, SkillName.AnimalLore, 20.0);
        ColdBonus = 10;
        EnergyBonus = 10;
        FireBonus = 10;
        PhysicalBonus = 10;
        PoisonBonus = 10;
        XmlAttach.AttachTo(this, new XmlLevelItem());
    }

    public BeastsWhisperersRobe(Serial serial) : base(serial)
    {
    }

    public override void Serialize(GenericWriter writer)
    {
        base.Serialize(writer);
        writer.Write((int)0); // version
    }

    public override void Deserialize(GenericReader reader)
    {
        base.Deserialize(reader);
        int version = reader.ReadInt();
    }
}
