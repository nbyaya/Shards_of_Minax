...
{
    // ---------------------------------------------------
    // RuneBeetle Summoning Materia
    // This materia, when socketed, gives a 99% chance to summon a RuneBeetle on hit
    // ---------------------------------------------------

    public class RuneBeetleSummoningMateria : BaseSocketAugmentation
    {
        private static readonly Random _random = new Random();

        [Constructable]
        public RuneBeetleSummoningMateria() : base(0x2809) // You can change the item ID to match your materia graphic
        {
            Name = "Summoning RuneBeetle Materia";
            Hue = _random.Next(1, 3001); // Generate a random hue between 1 and 3000
        }
        ...
        public RuneBeetleSummoningMateria(Serial serial) : base(serial)
        {
        }
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            return "Grants a 99% chance on hit to summon a RuneBeetle.";
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            if (target is BaseWeapon)
            {
                ...
                XmlAttach.AttachTo(target, new XmlSummonStrike("RuneBeetle", 99, 5.0));
                return true;
            }
            return false;
        }
