...
{
    // ---------------------------------------------------
    // Materia for XmlGlowLine
    // This materia, when socketed, allows the bearer to use XmlGlowLine ability
    // ---------------------------------------------------

    public class GlowLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public GlowLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "XmlGlowLine Materia";
        ...
        public GlowLineMateria(Serial serial) : base(serial)
        {
        }

        public override string OnIdentify(Mobile from)
        {
            return "Allows the bearer to use XmlGlowLine ability.";
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the XmlGlowLine effect with specified parameters
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlGlowLine());
                return true;
            }
            return false;
        }
