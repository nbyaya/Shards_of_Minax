...
namespace Server.Items
{
    // ---------------------------------------------------
    // Gate Line Materia
    // This materia, when socketed, allows the bearer to unleash a gate line attack
    // ---------------------------------------------------
    public class GateLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public GateLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "Gate Line Materia";
        ...
        public GateLineMateria(Serial serial) : base(serial)
        {
        }

        public override string OnIdentify(Mobile from)
        {
            return "Unleashes a powerful gate line attack on your foes.";
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the Gate Line effect with specified parameters (example: refractory 5 seconds, damage 20, range 8)
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlGateLine(5.0, 20, 8));
                return true;
            }
            return false;
        }
