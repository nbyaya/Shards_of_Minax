...
namespace Server.Items
{
    // ---------------------------------------------------
    // Spike Line Materia
    // This materia, when socketed, allows the bearer to unleash a spike line attack
    // ---------------------------------------------------

    public class SpikeLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public SpikeLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "Spike Line Materia";
        ...
        public SpikeLineMateria(Serial serial) : base(serial)
        {
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the Spike Line effect with specified parameters (example: refractory 5 seconds, damage 20, range 8)
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlSpikeLine(5.0, 20, 8));
                return true;
            }
            return false;
        }
