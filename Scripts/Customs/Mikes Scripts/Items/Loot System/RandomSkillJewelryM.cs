...
public class RandomSkillJewelryM : BaseJewel
{
    private static string[] prefixes = { "Dexterous", "Agile" };
    ...
    [Constructable]
    public RandomSkillJewelryM() : base(GetRandomItemID(), Layer.Ring)
    {
    ...
    private void InitializeJewelryAttributes(BaseJewel tempJewelry)
    {
        ...
        if (tierChance < 0.05) // Very rare
        {
            ...
            this.Attributes.ReflectPhysical = Utility.RandomMinMax(1, 20);
        }
        else if (tierChance < 0.2) // Rare
        {
            this.Attributes.Luck = rand.Next(40, 79);
            SkillBonuses.SetValues(0, SkillName.Fencing, Utility.RandomMinMax(0, 35));
            SkillBonuses.SetValues(1, SkillName.Tactics, Utility.RandomMinMax(0, 35));
            this.Attributes.ReflectPhysical = Utility.RandomMinMax(1, 15);
        }
        else if (tierChance < 0.5) // Uncommon
        {
            this.Attributes.Luck = rand.Next(20, 39);
            SkillBonuses.SetValues(0, SkillName.Fencing, Utility.RandomMinMax(0, 24));
            SkillBonuses.SetValues(1, SkillName.Tactics, Utility.RandomMinMax(0, 24));
            this.Attributes.ReflectPhysical = Utility.RandomMinMax(1, 10);
        }
        else // Common
        {
    ...
    public RandomSkillJewelryM(Serial serial) : base(serial) { }
