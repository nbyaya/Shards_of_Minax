...
namespace Server.Items
{
    public class BeggingAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which item would you like to augment for begging?");
        ...
        private class BeggingAugmentTarget : Target
        {
            ...
            private void AugmentItem(Mobile from, Item item, BeggingAugmentCrystal crystal)
            {
                const int maxSlots = 5; // Number of skill slots
                const SkillName beggingSkill = SkillName.Begging;
                ...
                if (existingSlot != -1)
                {
                    ...
                    if (value >= 25)
                    {
                        from.SendMessage("The item already has the maximum begging bonus.");
                        return;
                    }
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(existingSlot, beggingSkill, newValue);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(existingSlot, beggingSkill, newValue);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(existingSlot, beggingSkill, newValue);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(existingSlot, beggingSkill, newValue);

                    from.SendMessage($"The begging bonus has been increased to +{newValue}.");
                    crystal.Delete();
                }
                else if (emptySlot != -1)
                {
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(emptySlot, beggingSkill, bonus);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(emptySlot, beggingSkill, bonus);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(emptySlot, beggingSkill, bonus);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(emptySlot, beggingSkill, bonus);

                    from.SendMessage($"The item has been augmented with a +{bonus} begging bonus.");
                    crystal.Delete();
                }
                else
                {
                    from.SendMessage("All skill slots are occupied, and none can be used for begging.");
                }
            }
        }
    }
