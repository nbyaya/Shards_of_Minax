...
namespace Server.Items
{
    public class PeacemakingAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which item would you like to augment for peacemaking?");
        ...
        private class PeacemakingAugmentTarget : Target
        {
            ...
            private void AugmentItem(Mobile from, Item item, PeacemakingAugmentCrystal crystal)
            {
                const int maxSlots = 5; // Number of skill slots
                const SkillName peacemakingSkill = SkillName.Peacemaking;
                ...
                if (existingSlot != -1)
                {
                    ...
                    if (value >= 25)
                    {
                        from.SendMessage("The item already has the maximum peacemaking bonus.");
                        return;
                    }
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(existingSlot, peacemakingSkill, newValue);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(existingSlot, peacemakingSkill, newValue);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(existingSlot, peacemakingSkill, newValue);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(existingSlot, peacemakingSkill, newValue);

                    from.SendMessage($"The peacemaking bonus has been increased to +{newValue}.");
                    crystal.Delete();
                }
                else if (emptySlot != -1)
                {
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(emptySlot, peacemakingSkill, bonus);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(emptySlot, peacemakingSkill, bonus);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(emptySlot, peacemakingSkill, bonus);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(emptySlot, peacemakingSkill, bonus);

                    from.SendMessage($"The item has been augmented with a +{bonus} peacemaking bonus.");
                    crystal.Delete();
                }
                else
                {
                    from.SendMessage("All skill slots are occupied, and none can be used for peacemaking.");
                }
            }
        }
    }
