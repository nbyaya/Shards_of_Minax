...
namespace Server.Items
{
    public class BlacksmithyAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which armor or clothing item would you like to augment for Blacksmithy?");
        ...
        private class BlacksmithyAugmentTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is BaseArmor)
                {
                    ...
                    armor.SkillBonuses.SetValues(0, SkillName.Blacksmith, bonus); // Setting the random bonus.
                    from.SendMessage(String.Format("The armor has been augmented with a +{0} Blacksmithy bonus.", bonus));
                    m_Crystal.Delete();
                }
                else if (targeted is BaseClothing)
                {
                    BaseClothing clothing = targeted as BaseClothing;
                    clothing.SkillBonuses.SetValues(0, SkillName.Blacksmith, bonus); // Setting the random bonus.
                    from.SendMessage(String.Format("The clothing has been augmented with a +{0} Blacksmithy bonus.", bonus));
                    m_Crystal.Delete();
                }
                else
                {
                    from.SendMessage("You can't augment that for Blacksmithy.");
                }
            }
        }
    }
