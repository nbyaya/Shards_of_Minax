...
namespace Server.Items
{
    public class MageryAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which item would you like to augment for magery?");
        ...
        private class MageryAugmentTarget : Target
        {
            ...
            private void AugmentItem(Mobile from, Item item, MageryAugmentCrystal crystal)
            {
                const int maxSlots = 5; // Number of skill slots
                const SkillName magerySkill = SkillName.Magery;
                ...
                if (existingSlot != -1)
                {
                    ...
                    if (value >= 25)
                    {
                        from.SendMessage("The item already has the maximum magery bonus.");
                        return;
                    }
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(existingSlot, magerySkill, newValue);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(existingSlot, magerySkill, newValue);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(existingSlot, magerySkill, newValue);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(existingSlot, magerySkill, newValue);

                    from.SendMessage($"The magery bonus has been increased to +{newValue}.");
                    crystal.Delete();
                }
                else if (emptySlot != -1)
                {
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(emptySlot, magerySkill, bonus);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(emptySlot, magerySkill, bonus);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(emptySlot, magerySkill, bonus);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(emptySlot, magerySkill, bonus);

                    from.SendMessage($"The item has been augmented with a +{bonus} magery bonus.");
                    crystal.Delete();
                }
                else
                {
                    from.SendMessage("All skill slots are occupied, and none can be used for magery.");
                }
            }
        }
    }
