...
namespace Server.Items
{
    public class HealingAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which item would you like to augment for healing?");
        ...
        private class HealingAugmentTarget : Target
        {
            ...
            private void AugmentItem(Mobile from, Item item, HealingAugmentCrystal crystal)
            {
                const int maxSlots = 5; // Number of skill slots
                const SkillName healingSkill = SkillName.Healing;
                ...
                if (existingSlot != -1)
                {
                    ...
                    if (value >= 25)
                    {
                        from.SendMessage("The item already has the maximum healing bonus.");
                        return;
                    }
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(existingSlot, healingSkill, newValue);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(existingSlot, healingSkill, newValue);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(existingSlot, healingSkill, newValue);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(existingSlot, healingSkill, newValue);

                    from.SendMessage($"The healing bonus has been increased to +{newValue}.");
                    crystal.Delete();
                }
                else if (emptySlot != -1)
                {
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(emptySlot, healingSkill, bonus);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(emptySlot, healingSkill, bonus);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(emptySlot, healingSkill, bonus);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(emptySlot, healingSkill, bonus);

                    from.SendMessage($"The item has been augmented with a +{bonus} healing bonus.");
                    crystal.Delete();
                }
                else
                {
                    from.SendMessage("All skill slots are occupied, and none can be used for healing.");
                }
            }
        }
    }
