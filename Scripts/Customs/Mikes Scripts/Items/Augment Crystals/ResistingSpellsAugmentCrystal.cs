...
namespace Server.Items
{
    public class ResistingSpellsAugmentCrystal : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            from.SendMessage("Which item would you like to augment for resisting spells?");
        ...
        private class ResistingSpellsAugmentTarget : Target
        {
            ...
            private void AugmentItem(Mobile from, Item item, ResistingSpellsAugmentCrystal crystal)
            {
                const int maxSlots = 5; // Number of skill slots
                const SkillName resistingSpellsSkill = SkillName.MagicResist;
                ...
                if (existingSlot != -1)
                {
                    ...
                    if (value >= 25)
                    {
                        from.SendMessage("The item already has the maximum resisting spells bonus.");
                        return;
                    }
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(existingSlot, resistingSpellsSkill, newValue);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(existingSlot, resistingSpellsSkill, newValue);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(existingSlot, resistingSpellsSkill, newValue);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(existingSlot, resistingSpellsSkill, newValue);

                    from.SendMessage($"The resisting spells bonus has been increased to +{newValue}.");
                    crystal.Delete();
                }
                else if (emptySlot != -1)
                {
                    ...
                    if (item is BaseArmor armorToUpdate)
                        armorToUpdate.SkillBonuses.SetValues(emptySlot, resistingSpellsSkill, bonus);
                    else if (item is BaseClothing clothingToUpdate)
                        clothingToUpdate.SkillBonuses.SetValues(emptySlot, resistingSpellsSkill, bonus);
                    else if (item is BaseJewel jewelToUpdate)
                        jewelToUpdate.SkillBonuses.SetValues(emptySlot, resistingSpellsSkill, bonus);
                    else if (item is BaseWeapon weaponToUpdate)
                        weaponToUpdate.SkillBonuses.SetValues(emptySlot, resistingSpellsSkill, bonus);

                    from.SendMessage($"The item has been augmented with a +{bonus} resisting spells bonus.");
                    crystal.Delete();
                }
                else
                {
                    from.SendMessage("All skill slots are occupied, and none can be used for resisting spells.");
                }
            }
        }
    }
