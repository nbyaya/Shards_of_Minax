...
{
    public class DateTreeSeed : Item
    {
        [Constructable]
        public DateTreeSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a date seed";
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                HarvestDateTree tree = new HarvestDateTree();
        ...
        public DateTreeSeed(Serial serial) : base(serial)
        {
        }
