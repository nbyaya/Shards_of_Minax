...
{
    public class LimeSeed : Item
    {
        [Constructable]
        public LimeSeed() : base(0xF27) // Example seed graphic
        {
            Weight = 0.1;
            Name = "a lime seed";
        }
        ...
        private void PlantTarget(Mobile from, object targeted, object state)
        {
            ...
            if (from.Map.CanFit(loc.X, loc.Y, loc.Z, 16, false, true))
            {
                ...
                HarvestLimeTree tree = new HarvestLimeTree(); // Change to the desired tree type
        ...
        public LimeSeed(Serial serial) : base(serial)
        {
        }
