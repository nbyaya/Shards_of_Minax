...
{
    public class GamanFeeder : Barrel
    {
        private static List<GamanFeeder> Feeders = new List<GamanFeeder>();
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            list.Add("Place Limes inside to raise Gaman");
        }
        ...
        {
            foreach (GamanFeeder feeder in Feeders)
            {
                if (feeder != null && !feeder.Deleted)
                {
                    feeder.CheckAndSpawnGaman();
                }
            }
        }

        private void CheckAndSpawnGaman()
        {
            ...
            if (lime != null && lime.Amount > 0)
            {
                ...
                Point3D gamanLocation = GetRandomAdjacentEmptyLocation();
                if (Map != null && gamanLocation != Point3D.Zero)
                {
                    Gaman gaman = new Gaman();
                    gaman.MoveToWorld(gamanLocation, Map);
                }
            }
        }
        ...
        public GamanFeeder(Serial serial) : base(serial)
        {
        }
