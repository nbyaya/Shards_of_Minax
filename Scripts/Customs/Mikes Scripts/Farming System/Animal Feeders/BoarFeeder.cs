...
{
    public class BoarFeeder : Barrel
    {
        private static List<BoarFeeder> Feeders = new List<BoarFeeder>();
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            list.Add("Place Apples inside to raise boars");
        }
        ...
        {
            foreach (BoarFeeder feeder in Feeders)
            {
                if (feeder != null && !feeder.Deleted)
                {
                    feeder.CheckAndSpawnBoar();
                }
            }
        }

        private void CheckAndSpawnBoar()
        {
            ...
            if (apple != null && apple.Amount > 0)
            {
                ...
                Point3D boarLocation = GetRandomAdjacentEmptyLocation();
                if (Map != null && boarLocation != Point3D.Zero)
                {
                    Boar boar = new Boar();
                    boar.MoveToWorld(boarLocation, Map);
                }
            }
        }
        ...
        public BoarFeeder(Serial serial) : base(serial)
        {
        }
