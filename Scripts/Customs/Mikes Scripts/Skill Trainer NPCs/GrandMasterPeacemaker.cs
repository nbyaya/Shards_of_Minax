...
{
    public class GrandMasterPeacemaker : BaseCreature
    {
        [Constructable]
        public GrandMasterPeacemaker() : base(AIType.AI_Vendor, FightMode.None, 10, 1, 0.2, 0.4)
        {
            this.Name = "Ghandi";
            this.Title = "the GrandMaster Peacemaker";
        ...
        public GrandMasterPeacemaker(Serial serial) : base(serial)
        {
        }
        ...
        // Handle the interaction logic for training Peacemaking skill
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            ...
            {
                if (from.Skills.Peacemaking.Base < 120)
                {
                    from.Skills.Peacemaking.Base += 0.1; // Increase by 0.1, you can adjust as needed
                    from.SendMessage("Your Peacemaking skill has increased!");
                    dropped.Delete();
                    return true;
                }
                else
                {
                    from.SendMessage("You already possess great knowledge in Peacemaking.");
                    return false;
                }
            }
            else
