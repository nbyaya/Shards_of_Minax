...
{
    public class AnimalTamingContractGump : Gump
    {
        private AnimalTamingContract m_Contract;

        public AnimalTamingContractGump(Mobile from, AnimalTamingContract contract) : base(50, 50)
        {
            m_Contract = contract;

            AddPage(0);
            AddBackground(0, 0, 350, 250, 5170);
            AddLabel(75, 20, 0, "AnimalTaming Contract");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                from.Target = new AnimalTamingCreatureTarget(m_Contract);
            }
    ...
    public class AnimalTamingCreatureTarget : Target
    {
        private AnimalTamingContract m_Contract;

        public AnimalTamingCreatureTarget(AnimalTamingContract contract) : base(10, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            {
                if (creature.Controlled && creature.ControlMaster == from && creature.GetType() == AnimalTamingContractType.Get[m_Contract.CreatureType].Type)
                {
                    ...
                    {
                        from.SendMessage("You have completed the AnimalTaming contract!");
                        from.SendGump(new AnimalTamingContractGump(from, m_Contract));
                    }
                }
                else
                {
