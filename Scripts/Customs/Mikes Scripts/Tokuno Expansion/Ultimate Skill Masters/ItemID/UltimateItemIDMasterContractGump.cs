...
{
    public class UltimateItemIDMasterContractGump : Gump
    {
        private UltimateItemIDMasterContract m_Contract;

        public UltimateItemIDMasterContractGump(Mobile from, UltimateItemIDMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateItemIDItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current ItemID skill cap
                    double currentCap = 100.0;
                    Skill ItemID = from.Skills[SkillName.ItemID];
                    if (ItemID != null)
                        currentCap = ItemID.Cap;
                    ...
                    {
                        // Drop ItemID Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.ItemID, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your ItemID skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateItemIDMasterReward.GetRandomReward();
    ...
    public class UltimateItemIDItemTarget : Target
    {
        private UltimateItemIDMasterContract m_Contract;

        public UltimateItemIDItemTarget(UltimateItemIDMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
