...
{
    public class UltimateFletchingMasterContractGump : Gump
    {
        private UltimateFletchingMasterContract m_Contract;

        public UltimateFletchingMasterContractGump(Mobile from, UltimateFletchingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateFletchingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Fletching skill cap
                    double currentCap = 100.0;
                    Skill Fletching = from.Skills[SkillName.Fletching];
                    if (Fletching != null)
                        currentCap = Fletching.Cap;
                    ...
                    {
                        // Drop Fletching Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Fletching, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Fletching skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateFletchingMasterReward.GetRandomReward();
    ...
    public class UltimateFletchingItemTarget : Target
    {
        private UltimateFletchingMasterContract m_Contract;

        public UltimateFletchingItemTarget(UltimateFletchingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateFletchingItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
