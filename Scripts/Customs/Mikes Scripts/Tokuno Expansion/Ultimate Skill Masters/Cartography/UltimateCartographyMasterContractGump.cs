...
{
    public class UltimateCartographyMasterContractGump : Gump
    {
        private UltimateCartographyMasterContract m_Contract;

        public UltimateCartographyMasterContractGump(Mobile from, UltimateCartographyMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateCartographyItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Cartography skill cap
                    double currentCap = 100.0;
                    Skill Cartography = from.Skills[SkillName.Cartography];
                    if (Cartography != null)
                        currentCap = Cartography.Cap;
                    ...
                    {
                        // Drop Cartography Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Cartography, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Cartography skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateCartographyMasterReward.GetRandomReward();
    ...
    public class UltimateCartographyItemTarget : Target
    {
        private UltimateCartographyMasterContract m_Contract;

        public UltimateCartographyItemTarget(UltimateCartographyMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateCartographyItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
