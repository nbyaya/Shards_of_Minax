...
{
    public class UltimateLockpickingMasterContractGump : Gump
    {
        private UltimateLockpickingMasterContract m_Contract;

        public UltimateLockpickingMasterContractGump(Mobile from, UltimateLockpickingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateLockpickingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Lockpicking skill cap
                    double currentCap = 100.0;
                    Skill Lockpicking = from.Skills[SkillName.Lockpicking];
                    if (Lockpicking != null)
                        currentCap = Lockpicking.Cap;
                    ...
                    {
                        // Drop Lockpicking Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Lockpicking, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Lockpicking skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateLockpickingMasterReward.GetRandomReward();
    ...
    public class UltimateLockpickingItemTarget : Target
    {
        private UltimateLockpickingMasterContract m_Contract;

        public UltimateLockpickingItemTarget(UltimateLockpickingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
