...
{
    public class UltimateCookingMasterContractGump : Gump
    {
        private UltimateCookingMasterContract m_Contract;

        public UltimateCookingMasterContractGump(Mobile from, UltimateCookingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateCookingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Cooking skill cap
                    double currentCap = 100.0;
                    Skill Cooking = from.Skills[SkillName.Cooking];
                    if (Cooking != null)
                        currentCap = Cooking.Cap;
                    ...
                    {
                        // Drop Cooking Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Cooking, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Cooking skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateCookingMasterReward.GetRandomReward();
    ...
    public class UltimateCookingItemTarget : Target
    {
        private UltimateCookingMasterContract m_Contract;

        public UltimateCookingItemTarget(UltimateCookingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateCookingItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
