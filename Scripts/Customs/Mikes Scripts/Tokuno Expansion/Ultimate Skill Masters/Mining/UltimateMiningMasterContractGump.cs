...
{
    public class UltimateMiningMasterContractGump : Gump
    {
        private UltimateMiningMasterContract m_Contract;

        public UltimateMiningMasterContractGump(Mobile from, UltimateMiningMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateMiningItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Mining skill cap
                    double currentCap = 100.0;
                    Skill Mining = from.Skills[SkillName.Mining];
                    if (Mining != null)
                        currentCap = Mining.Cap;
                    ...
                    {
                        // Drop Mining Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Mining, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Mining skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateMiningMasterReward.GetRandomReward();
    ...
    public class UltimateMiningItemTarget : Target
    {
        private UltimateMiningMasterContract m_Contract;

        public UltimateMiningItemTarget(UltimateMiningMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateMiningItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
