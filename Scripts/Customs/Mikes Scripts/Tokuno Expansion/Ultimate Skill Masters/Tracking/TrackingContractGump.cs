...
{
    public class TrackingContractGump : Gump
    {
        private TrackingContract m_Contract;

        public TrackingContractGump(Mobile from, TrackingContract contract) : base(50, 50)
        {
            m_Contract = contract;

            AddPage(0);
            AddBackground(0, 0, 350, 250, 5170);
            AddLabel(75, 20, 0, "Tracking Contract");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                from.Target = new TrackingCreatureTarget(m_Contract);
            }
    ...
    public class TrackingCreatureTarget : Target
    {
        private TrackingContract m_Contract;

        public TrackingCreatureTarget(TrackingContract contract) : base(10, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            {
                if (creature.Controlled && creature.ControlMaster == from && creature.GetType() == TrackingContractType.Get[m_Contract.CreatureType].Type)
                {
                    ...
                    {
                        from.SendMessage("You have completed the Tracking contract!");
                        from.SendGump(new TrackingContractGump(from, m_Contract));
                    }
                }
                else
                {
