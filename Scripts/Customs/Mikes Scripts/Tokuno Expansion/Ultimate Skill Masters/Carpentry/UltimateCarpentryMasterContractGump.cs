...
{
    public class UltimateCarpentryMasterContractGump : Gump
    {
        private UltimateCarpentryMasterContract m_Contract;

        public UltimateCarpentryMasterContractGump(Mobile from, UltimateCarpentryMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateCarpentryItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Carpentry skill cap
                    double currentCap = 100.0;
                    Skill Carpentry = from.Skills[SkillName.Carpentry];
                    if (Carpentry != null)
                        currentCap = Carpentry.Cap;
                    ...
                    {
                        // Drop Carpentry Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Carpentry, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Carpentry skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateCarpentryMasterReward.GetRandomReward();
    ...
    public class UltimateCarpentryItemTarget : Target
    {
        private UltimateCarpentryMasterContract m_Contract;

        public UltimateCarpentryItemTarget(UltimateCarpentryMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateCarpentryItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
