...
{
    public class CampingContractGump : Gump
    {
        private CampingContract m_Contract;

        public CampingContractGump(Mobile from, CampingContract contract) : base(50, 50)
        {
            m_Contract = contract;

            AddPage(0);
            AddBackground(0, 0, 350, 250, 5170);
            AddLabel(75, 20, 0, "Camping Contract");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                from.Target = new CampingCreatureTarget(m_Contract);
            }
    ...
    public class CampingCreatureTarget : Target
    {
        private CampingContract m_Contract;

        public CampingCreatureTarget(CampingContract contract) : base(10, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            {
                if (creature.Controlled && creature.ControlMaster == from && creature.GetType() == CampingContractType.Get[m_Contract.CreatureType].Type)
                {
                    ...
                    {
                        from.SendMessage("You have completed the Camping contract!");
                        from.SendGump(new CampingContractGump(from, m_Contract));
                    }
                }
                else
                {
