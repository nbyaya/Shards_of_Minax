...
{
    public class UltimateTasteIDMasterContractGump : Gump
    {
        private UltimateTasteIDMasterContract m_Contract;

        public UltimateTasteIDMasterContractGump(Mobile from, UltimateTasteIDMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateTasteIDItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current TasteID skill cap
                    double currentCap = 100.0;
                    Skill TasteID = from.Skills[SkillName.TasteID];
                    if (TasteID != null)
                        currentCap = TasteID.Cap;
                    ...
                    {
                        // Drop TasteID Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.TasteID, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your TasteID skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateTasteIDMasterReward.GetRandomReward();
    ...
    public class UltimateTasteIDItemTarget : Target
    {
        private UltimateTasteIDMasterContract m_Contract;

        public UltimateTasteIDItemTarget(UltimateTasteIDMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateTasteIDItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
