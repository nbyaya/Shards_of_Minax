...
{
    public class UltimateAlchemyMasterContractGump : Gump
    {
        private UltimateAlchemyMasterContract m_Contract;

        public UltimateAlchemyMasterContractGump(Mobile from, UltimateAlchemyMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateAlchemyItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Alchemy skill cap
                    double currentCap = 100.0;
                    Skill alchemy = from.Skills[SkillName.Alchemy];
                    if (alchemy != null)
                        currentCap = alchemy.Cap;
                    ...
                    {
                        // Drop Alchemy Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Alchemy, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Alchemy skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateAlchemyMasterReward.GetRandomReward();
    ...
    public class UltimateAlchemyItemTarget : Target
    {
        private UltimateAlchemyMasterContract m_Contract;

        public UltimateAlchemyItemTarget(UltimateAlchemyMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateAlchemyItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
