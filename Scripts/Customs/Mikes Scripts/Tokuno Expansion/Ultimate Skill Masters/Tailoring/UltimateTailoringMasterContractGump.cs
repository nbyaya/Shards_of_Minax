...
{
    public class UltimateTailoringMasterContractGump : Gump
    {
        private UltimateTailoringMasterContract m_Contract;

        public UltimateTailoringMasterContractGump(Mobile from, UltimateTailoringMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;

            AddPage(0);
            AddBackground(0, 0, 300, 200, 5170);
            AddLabel(40, 40, 0, $"Collect {contract.AmountNeeded} {contract.ItemType}(s)");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateTailoringItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Tailoring skill cap
                    double currentCap = 100.0;
                    Skill Tailoring = from.Skills[SkillName.Tailoring];
                    if (Tailoring != null)
                        currentCap = Tailoring.Cap;
                    ...
                    {
                        // Drop Tailoring Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Tailoring, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Tailoring skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateTailoringMasterReward.GetRandomReward();
    ...
    public class UltimateTailoringItemTarget : Target
    {
        private UltimateTailoringMasterContract m_Contract;

        public UltimateTailoringItemTarget(UltimateTailoringMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateTailoringItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
