...
{
    public class UltimateLumberjackingMasterContractGump : Gump
    {
        private UltimateLumberjackingMasterContract m_Contract;

        public UltimateLumberjackingMasterContractGump(Mobile from, UltimateLumberjackingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateLumberjackingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Lumberjacking skill cap
                    double currentCap = 100.0;
                    Skill Lumberjacking = from.Skills[SkillName.Lumberjacking];
                    if (Lumberjacking != null)
                        currentCap = Lumberjacking.Cap;
                    ...
                    {
                        // Drop Lumberjacking Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Lumberjacking, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Lumberjacking skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateLumberjackingMasterReward.GetRandomReward();
    ...
    public class UltimateLumberjackingItemTarget : Target
    {
        private UltimateLumberjackingMasterContract m_Contract;

        public UltimateLumberjackingItemTarget(UltimateLumberjackingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateLumberjackingItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
