...
{
    public class UltimateInscribeMasterContractGump : Gump
    {
        private UltimateInscribeMasterContract m_Contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateInscribeItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Inscribe skill cap
                    double currentCap = 100.0;
                    Skill Inscribe = from.Skills[SkillName.Inscribe];
                    if (Inscribe != null)
                        currentCap = Inscribe.Cap;
                    ...
                    {
                        // Drop Inscribe Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Inscribe, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Inscribe skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateInscribeMasterReward.GetRandomReward();
    ...
    public class UltimateInscribeItemTarget : Target
    {
        private UltimateInscribeMasterContract m_Contract;

        public UltimateInscribeItemTarget(UltimateInscribeMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateInscribeItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
