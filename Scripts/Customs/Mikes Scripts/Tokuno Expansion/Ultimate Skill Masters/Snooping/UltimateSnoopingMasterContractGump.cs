...
{
    public class UltimateSnoopingMasterContractGump : Gump
    {
        private UltimateSnoopingMasterContract m_Contract;

        public UltimateSnoopingMasterContractGump(Mobile from, UltimateSnoopingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateSnoopingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Snooping skill cap
                    double currentCap = 100.0;
                    Skill Snooping = from.Skills[SkillName.Snooping];
                    if (Snooping != null)
                        currentCap = Snooping.Cap;
                    ...
                    {
                        // Drop Snooping Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Snooping, newCap);
                        from.BankBox.DropItem(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Snooping skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateSnoopingMasterReward.GetRandomReward();
    ...
    public class UltimateSnoopingItemTarget : Target
    {
        private UltimateSnoopingMasterContract m_Contract;

        public UltimateSnoopingItemTarget(UltimateSnoopingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
