...
{
    public class UltimateHealingMasterContractGump : Gump
    {
        private UltimateHealingMasterContract m_Contract;

        public UltimateHealingMasterContractGump(Mobile from, UltimateHealingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateHealingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Healing skill cap
                    double currentCap = 100.0;
                    Skill Healing = from.Skills[SkillName.Healing];
                    if (Healing != null)
                        currentCap = Healing.Cap;
                    ...
                    {
                        // Drop Healing Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Healing, newCap);
                        from.AddToBackpack(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Healing skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateHealingMasterReward.GetRandomReward();
    ...
    public class UltimateHealingItemTarget : Target
    {
        private UltimateHealingMasterContract m_Contract;

        public UltimateHealingItemTarget(UltimateHealingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateHealingItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
