...
{
    public class UltimateImbuingMasterContractGump : Gump
    {
        private UltimateImbuingMasterContract m_Contract;

        public UltimateImbuingMasterContractGump(Mobile from, UltimateImbuingMasterContract contract) : base(0, 0)
        {
            m_Contract = contract;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Add Item
            {
                ...
                from.Target = new UltimateImbuingItemTarget(m_Contract);
            }
            else if (info.ButtonID == 2) // Claim Reward
            {
                if (m_Contract.AmountCollected >= m_Contract.AmountNeeded)
                {
                    ...
                    // Determine player's current Imbuing skill cap
                    double currentCap = 100.0;
                    Skill Imbuing = from.Skills[SkillName.Imbuing];
                    if (Imbuing != null)
                        currentCap = Imbuing.Cap;
                    ...
                    {
                        // Drop Imbuing Powerscroll
                        PowerScroll ps = new PowerScroll(SkillName.Imbuing, newCap);
                        from.BankBox.DropItem(ps);
                    }
                    else
                    {
                        from.SendMessage("Your Imbuing skill is already at or above the maximum (150). No skill scroll given.");
                    }

                    // Drop a special random reward
                    Item specialReward = UltimateImbuingMasterReward.GetRandomReward();
    ...
    public class UltimateImbuingItemTarget : Target
    {
        private UltimateImbuingMasterContract m_Contract;

        public UltimateImbuingItemTarget(UltimateImbuingMasterContract contract) : base(1, false, TargetFlags.None)
        {
            m_Contract = contract;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item item && item.GetType() == m_Contract.ItemType && item.IsChildOf(from.Backpack))
            {
                ...
                else
                {
                    ...
                    from.Target = new UltimateImbuingItemTarget(m_Contract); // Reinitialize the targeting
                }
            }
            else
            {
