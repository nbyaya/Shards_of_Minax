...
{
    public class HeartwoodAxe : Item
    {
        public override string DefaultName
        {
            get { return "a heartwood axe"; }
        }

        private int m_Uses;

        [Constructable]
        public HeartwoodAxe() : base(0xF43) // Axe item ID
        {
            Weight = 5.0;
            Hue = 0x42F; // Unique color for the axe
            m_Uses = 50; // Total uses
        }

        public HeartwoodAxe(Serial serial) : base(serial)
        {
        }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new HeartwoodLog()); // OakLog item to be created
                    m_Uses--;
