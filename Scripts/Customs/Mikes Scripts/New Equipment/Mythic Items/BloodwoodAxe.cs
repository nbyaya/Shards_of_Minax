...
{
    public class BloodwoodAxe : Item
    {
        public override string DefaultName
        {
            get { return "a bloodwood axe"; }
        }

        private int m_Uses;

        [Constructable]
        public BloodwoodAxe() : base(0xF43) // Axe item ID
        {
            Weight = 5.0;
            Hue = 0x440; // Unique color for the axe
            m_Uses = 50; // Total uses
        }

        public BloodwoodAxe(Serial serial) : base(serial)
        {
        }
        ...
        private void OnTarget(Mobile from, object targeted)
        {
            if (targeted is StaticTarget)
            {
                ...
                {
                    from.AddToBackpack(new BloodwoodLog()); // OakLog item to be created
                    m_Uses--;
