...
{
    public class MonsterChestX : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestX() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int starCitizenCount = Utility.Random(1, 6);

            for (int i = 0; i < starCitizenCount; i++)
                monsters.Add(new StarCitizen()); // Assuming you have a StarCitizen class defined
        ...
        public MonsterChestX(Serial serial) : base(serial)
        {
        }
