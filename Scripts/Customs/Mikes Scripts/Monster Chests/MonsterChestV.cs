...
{
    public class MonsterChestV : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestV() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int evilAlchemistCount = Utility.Random(1, 6);

            for (int i = 0; i < evilAlchemistCount; i++)
                monsters.Add(new EvilAlchemist());
        ...
        public MonsterChestV(Serial serial) : base(serial)
        {
        }
