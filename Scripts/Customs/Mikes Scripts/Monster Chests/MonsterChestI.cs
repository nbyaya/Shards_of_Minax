...
{
    public class MonsterChestI : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestI() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int druidCount = Utility.Random(1, 6);

            for (int i = 0; i < druidCount; i++)
                monsters.Add(new HostileDruid());
        ...
        public MonsterChestI(Serial serial) : base(serial)
        {
        }
