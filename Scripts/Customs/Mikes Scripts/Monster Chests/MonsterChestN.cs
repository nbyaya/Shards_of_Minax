...
{
    public class MonsterChestN : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestN() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int cultistCount = Utility.Random(1, 6);

            for (int i = 0; i < cultistCount; i++)
                monsters.Add(new TwistedCultist());
        ...
        public MonsterChestN(Serial serial) : base(serial)
        {
        }
