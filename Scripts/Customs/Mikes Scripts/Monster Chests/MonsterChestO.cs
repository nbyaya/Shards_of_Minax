...
{
    public class MonsterChestO : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestO() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int witchCount = Utility.Random(1, 6);

            for (int i = 0; i < witchCount; i++)
                monsters.Add(new MischievousWitch());
        ...
        public MonsterChestO(Serial serial) : base(serial)
        {
        }
