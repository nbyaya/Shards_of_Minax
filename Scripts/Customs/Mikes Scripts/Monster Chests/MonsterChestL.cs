...
{
    public class MonsterChestL : WoodenChest
    {
        private bool m_Activated = true;

        [Constructable]
        public MonsterChestL() : base()
        {
            this.Movable = true;
        }
        ...
        private void SpawnCreatures(Mobile from)
        {
            ...
            int lawyerCount = Utility.Random(1, 6);

            for (int i = 0; i < lawyerCount; i++)
                monsters.Add(new Lawyer());
        ...
        public MonsterChestL(Serial serial) : base(serial)
        {
        }
