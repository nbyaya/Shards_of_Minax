...
{
    public class GrandMasterAlchemist : BaseCreature
    {
        [Constructable]
        public GrandMasterAlchemist() : base(AIType.AI_Vendor, FightMode.None, 10, 1, 0.2, 0.4)
        {
            this.Name = "Merlin";
            this.Title = "the GrandMaster Alchemist";
        ...
        public GrandMasterAlchemist(Serial serial) : base(serial)
        {
        }
        ...
        // Handle the interaction logic for training Alchemy skill
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            ...
            {
                if (from.Skills.Alchemy.Base < 120)
                {
                    from.Skills.Alchemy.Base += 0.1; // Increase by 0.1, you can adjust as needed
                    from.SendMessage("Your Alchemy skill has increased!");
                    dropped.Delete();
                    return true;
                }
                else
                {
                    from.SendMessage("You already possess great knowledge in Alchemy.");
                    return false;
                }
            }
            else
