...
{
    public class GrandMasterProvocationTrainer : BaseCreature
    {
        [Constructable]
        public GrandMasterProvocationTrainer() : base(AIType.AI_Vendor, FightMode.None, 10, 1, 0.2, 0.4)
        {
            this.Name = "Ozzy";
            this.Title = "the GrandMaster Provocation Trainer";
        ...
        public GrandMasterProvocationTrainer(Serial serial) : base(serial)
        {
        }
        ...
        // Handle the interaction logic for training Provocation skill
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            ...
            {
                if (from.Skills.Provocation.Base < 120)
                {
                    from.Skills.Provocation.Base += 0.1; // Increase by 0.1, you can adjust as needed
                    from.SendMessage("Your Provocation skill has increased!");
                    dropped.Delete();
                    return true;
                }
                else
                {
                    from.SendMessage("You already possess great knowledge in Provocation.");
                    return false;
                }
            }
            else
