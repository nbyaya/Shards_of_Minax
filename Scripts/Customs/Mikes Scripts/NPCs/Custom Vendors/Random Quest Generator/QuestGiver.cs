...
namespace Server.Mobiles
{
    ...
    public class QuestGiver : Mobile
    {
        ...
        // New method to generate a random Powerscroll
        private Item GenerateRandomPowerscroll()
        {
            SkillName skill = GetRandomSkillName();
            int value = Utility.RandomMinMax(51, 90); // Random skill value between 51 and 90
            return new PowerScroll(skill, value);
        }

        // Custom method to get a random SkillName
        private SkillName GetRandomSkillName()
        {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            ...
            if (mobile != null)
            {
                ...
                else if (dropped is QuestScroll)
                {
                    ...
                    // Always give a powerscroll as part of the reward
                    mobile.AddToBackpack(GenerateRandomPowerscroll());

                    if (Utility.RandomMinMax(1, 4) == 1)
                    {
                        mobile.AddToBackpack(new Gold(m_Quest.NLevel * Utility.RandomMinMax(1000, 2000)));
                        mobile.AddToBackpack(new MaxxiaScroll());
                    }
                    else
                    {
                        mobile.AddToBackpack(new Gold(m_Quest.NLevel * Utility.RandomMinMax(1000, 2000)));
...
namespace Server.Gumps
{
    public class QuestGiver_gump : Gump
    {
        ...
        public QuestGiver_gump(Mobile owner) : base(50, 50)
        {
            ...
            "<BASEFONT COLOR=YELLOW><BR>" +
            "<BASEFONT COLOR=YELLOW>Rewards very from gold, items,<BR>" +
            "<BASEFONT COLOR=YELLOW>Maxxia Scrolls, and Power Scrolls.<BR>" +
            "</BODY>", false, true);
