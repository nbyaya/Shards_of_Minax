...
{
    public class PowerscrollVendor
    {
        public static void Initialize()
        {
            CommandSystem.Register("openpowerscrolls", AccessLevel.Player, new CommandEventHandler(OpenPowerscrolls_OnCommand));
        }

        [Usage("OpenPowerscrolls")]
        [Description("Opens a visual representation of available Powerscroll 75.")]
    ...
    public class PowerscrollVendorGump : Gump
    {
        ...
        public PowerscrollVendorGump(PlayerMobile owner, int page) : base(50, 50)
        {
            m_Owner = owner;
            m_Page = page;
            ...
            AddHtml(20, 20, 560, 20, "<CENTER><BASEFONT COLOR=#FFFFFF>Powerscroll 75 Vendor</BASEFONT></CENTER>", false, false);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                case 1: // Previous page
                    from.SendGump(new PowerscrollVendorGump(m_Owner, m_Page - 1));
                    break;
                case 2: // Next page
                    from.SendGump(new PowerscrollVendorGump(m_Owner, m_Page + 1));
                    break;
                    ...
                    if (info.ButtonID >= 100)
                    {
                        ...
                        if (index >= 0 && index < AllSkills.Count)
                        {
                            ...
                            if (HasEnoughMaxxiaScrolls(from))
                            {
                                ...
                                from.SendMessage("You have purchased a Powerscroll 75 for {0}.", skill);
                            }
                            else
                            {
                                from.SendMessage("You don't have enough MaxxiaScrolls. You need 2 to purchase a Powerscroll 75.");
                            }
                        }
                        from.SendGump(new PowerscrollVendorGump(m_Owner, m_Page));
                    }
                    break;
            }
        }
        ...
        {
            from.Backpack.ConsumeTotal(typeof(MaxxiaScroll), 2);
        }

        private void GivePowerscroll(Mobile from, SkillName skill)
        {
            PowerScroll ps = new PowerScroll(skill, 75.0);
