...
{
    public class ActsOfHonorQuest : BaseQuest
    {
        public override bool DoneOnce { get { return false; } } // Repeatable
        public override object Title { get { return "Acts of Honor"; } }
        public override object Description
        {
            get
            {
                return "Greetings, brave one! Our land is in need of heroes. " +
                       "You are tasked with either slaying fearsome creatures or " +
                       "collecting precious artifacts. Complete your task and return to me for a reward.";
            }
        }

        public override object Refuse { get { return "Understood. Should you decide to accept the quest, find me again."; } }

        public override object Uncomplete { get { return "The land still needs your bravery. Please fulfill your quest."; } }

        public override object Complete { get { return "Your honor has been proven. Here is your reward."; } }
        ...
        public ActsOfHonorQuest() : base()
        {
            // Randomly decide between kill or collect objective
            if (Utility.RandomBool())
            {
                // Kill Objective
                AddObjective(new SlayObjective(typeof(FierceDragon), "Fierce Dragon", 1));
            }
            else
            {
                // Collection Objective
                AddObjective(new ObtainObjective(typeof(AncientArtifact), "Ancient Artifact", 1, 0x1F3)); // 0x1F3 is an example item ID
            }

            // Reward: Honor Stone
            AddReward(new BaseReward(typeof(HonorStone), 1, "1 Honor Stone"));
        ...
        public override void OnAccept()
        {
            ...
            {
                // Spawn a Fierce Dragon
                m_Target = new FierceDragon();
                m_Target.MoveToWorld(location, map);

                objectiveMessage = "A Fierce Dragon has been sighted at " + location + " on the Felucca map. Find it and defeat it.";
                Owner.SendMessage(objectiveMessage);
            }
            else if (Objectives[0] is ObtainObjective)
            {
                // Spawn Ancient Artifacts
                SpawnAncientArtifacts(location, map);

                objectiveMessage = "Ancient Artifacts have been scattered around " + location + " on the Felucca map. Gather them.";
                Owner.SendMessage(objectiveMessage);
            }
        ...
        private void SpawnAncientArtifacts(Point3D location, Map map)
        {
            for (int i = 0; i < 3; i++) // Adjust the number if you want to spawn more artifacts
            {
                Item artifact = new AncientArtifact();
                artifact.MoveToWorld(location, map);
            }
        }
        ...
        {
            base.OnCompleted();
            Owner.SendMessage("Your actions have honored our land.");
