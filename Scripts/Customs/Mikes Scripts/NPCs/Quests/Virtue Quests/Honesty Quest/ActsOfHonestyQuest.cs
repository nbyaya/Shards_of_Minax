...
{
    public class ActsOfHonestyQuest : BaseQuest
    {
        public override bool DoneOnce { get { return false; } } // Repeatable
        public override object Title { get { return "Acts of Honesty"; } }
        public override object Description
        {
            get
            {
                return "Greetings, noble adventurer! To uphold the virtue of Honesty, you must either recover " +
                       "lost coins from the land or deal with deceitful creatures that spread falsehoods. " +
                       "Complete your task and return to me for your reward.";
            }
        }

        public override object Refuse { get { return "I understand. If you change your mind, come back and see me."; } }

        public override object Uncomplete { get { return "The deception continues. Please complete your mission."; } }

        public override object Complete { get { return "Your honesty has been rewarded. Here is your reward."; } }
        ...
        public ActsOfHonestyQuest() : base()
        {
            // Randomly decide between kill or collect objective
            if (Utility.RandomBool())
            {
                // Kill Objective
                AddObjective(new SlayObjective(typeof(DeceitfulCreature), "Deceitful Creature", 1));
            }
            else
            {
                // Collection Objective
                AddObjective(new ObtainObjective(typeof(LostCoin), "Lost Coins", 5, 0x1B9)); // 0x1B9 is an example item ID
            }

            // Reward: Honesty Stone
            AddReward(new BaseReward(typeof(HonestyStone), 1, "1 Honesty Stone"));
        ...
        public override void OnAccept()
        {
            ...
            {
                // Spawn a Deceitful Creature
                m_Target = new DeceitfulCreature();
                m_Target.MoveToWorld(location, map);

                objectiveMessage = "A Deceitful Creature has been spotted at " + location + " on the Ilshenar map. Seek it out and eliminate it.";
                Owner.SendMessage(objectiveMessage);
            }
            else if (Objectives[0] is ObtainObjective)
            {
                // Spawn Lost Coins
                SpawnLostCoins(location, map);

                objectiveMessage = "Lost Coins have been scattered around " + location + " on the Ilshenar map. Please collect them.";
                Owner.SendMessage(objectiveMessage);
            }
        ...
        private void SpawnLostCoins(Point3D location, Map map)
        {
            for (int i = 0; i < 5; i++) // Adjust the number if you want to spawn more coins
            {
                Item coin = new LostCoin();
                coin.MoveToWorld(location, map);
            }
        }
        ...
        {
            base.OnCompleted();
            Owner.SendMessage("Your honest efforts have been rewarded.");
