...
{
    public class ActsOfValorQuest : BaseQuest
    {
        public override bool DoneOnce { get { return false; } } // Repeatable
        public override object Title { get { return "Acts of Valor"; } }
        public override object Description
        {
            get
            {
                return "Brave warrior, our land is threatened by dangerous creatures. " +
                       "Your task is to either eliminate these threats or gather valuable items " +
                       "that will aid us in our struggle. Once you have completed your task, return to me for your reward.";
            }
        }

        public override object Refuse { get { return "I understand. If you change your mind, seek me out."; } }

        public override object Uncomplete { get { return "The threats remain. Please complete your task."; } }

        public override object Complete { get { return "Your valor has helped us greatly. Here is your reward."; } }
        ...
        public ActsOfValorQuest() : base()
        {
            // Randomly decide between kill or collect objective
            if (Utility.RandomBool())
            {
                // Kill Objective
                AddObjective(new SlayObjective(typeof(DreadfulBeast), "Dreadful Beast", 1));
            }
            else
            {
                // Collection Objective
                AddObjective(new ObtainObjective(typeof(ValorItem), "Valor Item", 1, 0xF05)); // 0xF05 is an example item ID
            }

            // Reward: Valor Stone
            AddReward(new BaseReward(typeof(ValorStone), 1, "1 Valor Stone"));
        ...
        public override void OnAccept()
        {
            ...
            {
                // Spawn a Dreadful Beast
                m_Target = new DreadfulBeast();
                m_Target.MoveToWorld(location, map);

                objectiveMessage = "A Dreadful Beast has been spotted at " + location + " on the Ilshenar map. Seek it out and eliminate the threat.";
                Owner.SendMessage(objectiveMessage);
            }
            else if (Objectives[0] is ObtainObjective)
            {
                // Spawn Valor Items
                SpawnValorItems(location, map);

                objectiveMessage = "Valor Items have been scattered around " + location + " on the Ilshenar map. Please collect them.";
                Owner.SendMessage(objectiveMessage);
            }
        ...
        private void SpawnValorItems(Point3D location, Map map)
        {
            for (int i = 0; i < 5; i++) // Adjust the number if you want to spawn more items
            {
                Item item = new ValorItem();
        ...
        {
            base.OnCompleted();
            Owner.SendMessage("Your valorous actions have made a difference.");
