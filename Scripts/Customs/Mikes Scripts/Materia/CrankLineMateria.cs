...
namespace Server.Items
{
    // ---------------------------------------------------
    // Crank Line Materia
    // This materia, when socketed, allows the bearer to unleash a crank line attack
    // ---------------------------------------------------

    public class CrankLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public CrankLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "Crank Line Materia";
        ...
        public CrankLineMateria(Serial serial) : base(serial)
        {
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the Crank Line effect with specified parameters (example: refractory 5 seconds, damage 20, range 8)
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlCrankLine(5.0, 20, 8));
                return true;
            }
            return false;
        }
