...
namespace Server.Items
{
    // ---------------------------------------------------
    // FTree Line Materia
    // This materia, when socketed, allows the bearer to unleash an FTree line attack
    // ---------------------------------------------------

    public class FTreeLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public FTreeLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "FTree Line Materia";
        ...
        public FTreeLineMateria(Serial serial) : base(serial)
        {
        }

        public override string OnIdentify(Mobile from)
        {
            return "Unleashes a powerful line attack on your foes.";
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the FTree Line effect with specified parameters (example: refractory 5 seconds, damage 20, range 8)
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlFTreeLine(5.0, 20, 8));
                return true;
            }
            return false;
        }
