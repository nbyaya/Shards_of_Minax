...
namespace Server.Items
{
    // ---------------------------------------------------
    // Axe Line Materia
    // This materia, when socketed, allows the bearer to unleash an axe line attack
    // ---------------------------------------------------

    public class AxeLineMateria : BaseSocketAugmentation
    {
        [Constructable]
        public AxeLineMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "Axe Line Materia";
        ...
        public AxeLineMateria(Serial serial) : base(serial)
        {
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the Axe Line effect with specified parameters (example: refractory 5 seconds, damage 20, range 8)
                // Adjust these parameters based on the desired effect
                XmlAttach.AttachTo(target, new XmlAxeLine(5.0, 20, 8));
                return true;
            }
            return false;
        }
