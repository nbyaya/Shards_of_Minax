...
namespace Server.Items
{
    // ---------------------------------------------------
    // Trap Circle Materia
    // This materia, when socketed, allows the bearer to set a trap that damages enemies within its radius
    // ---------------------------------------------------

    public class TrapCircleMateria : BaseSocketAugmentation
    {
        [Constructable]
        public TrapCircleMateria() : base(0x2809) // Use the item ID for your materia graphic
        {
            Name = "Trap Circle Materia";
        ...
        public TrapCircleMateria(Serial serial) : base(serial)
        {
        }

        public override string OnIdentify(Mobile from)
        {
            return "Sets a trap that damages all foes within its radius.";
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            {
                // Attach the Trap Circle effect with specified parameters (example: refractory 15 seconds, damage 20, radius 5, thickness 1)
                XmlAttach.AttachTo(target, new XmlTrapCircle(15.0, 20, 5, 1));
                return true;
            }
            return false;
        }
