...
namespace Server.Items
{
	...
	public class WorldOmniporter : Item
	{
		...
		public static void Initialize()
		{
			...
			{
				new OmniEntry("Britain", new Point3D(1336, 1997, 5), Map.Trammel ),
				new OmniEntry( "New Haven", new Point3D(3450, 2677, 25), Map.Trammel ),
				new OmniEntry("Jhelom", new Point3D(1499, 3771, 5), Map.Trammel ),
				new OmniEntry("Magincia", new Point3D(3563, 2139, 34), Map.Trammel ),
				new OmniEntry("Minoc", new Point3D(2701, 692, 5), Map.Trammel ),
				new OmniEntry("Moonglow", new Point3D(4467, 1283, 5), Map.Trammel ),
				new OmniEntry("Skara Brae", new Point3D(643, 2067, 5), Map.Trammel ),
				new OmniEntry("Trinsic", new Point3D(1828, 2948, -20), Map.Trammel ),
				new OmniEntry("Yew", new Point3D(771, 752, 5), Map.Trammel ),
				new OmniEntry("Britain", new Point3D(1336, 1997, 5), Map.Felucca ),
				new OmniEntry("Buccaneer's Den", new Point3D(2711, 2234, 0), Map.Felucca ),
				new OmniEntry("Jhelom", new Point3D(1499, 3771, 5), Map.Felucca ),
				new OmniEntry("Magincia", new Point3D(3563, 2139, 34), Map.Felucca ),
				new OmniEntry("Minoc", new Point3D(2701, 692, 5), Map.Felucca ),
				new OmniEntry("Moonglow", new Point3D(4467, 1283, 5), Map.Felucca ),
				new OmniEntry("Skara Brae", new Point3D(643, 2067, 5), Map.Felucca ),
				new OmniEntry("Trinsic", new Point3D(1828, 2948, -20), Map.Felucca ),
				new OmniEntry("Yew", new Point3D(771, 752, 5), Map.Felucca )
	...
	public class WorldOmniporterGump : Gump
	{
		...
		public bool HasRequiredItem(Container container, Type requiredItemType)
		{
			// Check if the container itself has the item
			foreach (Item item in container.Items)
			{
				if (item.GetType() == requiredItemType)
					return true;

				// If the item is a container, recursively search within it
				if (item is Container subContainer)
				{
					if (HasRequiredItem(subContainer, requiredItemType))
						return true;
				}
			}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if (m_Entry.RequiredItem != null && !HasRequiredItem(from.Backpack, m_Entry.RequiredItem))
			{
				from.SendMessage("You need the required item to travel to this destination.");
				return;
			}
	...
	public class OmniEntry
	{
		...
		// New property for the required item type
		private Type m_RequiredItem;
		public Type RequiredItem{ get { return m_RequiredItem; } set { m_RequiredItem = value; } }

		public OmniEntry( string name, Point3D p, Map map, Type requiredItem = null, bool moongate = false)
		{
			m_Name = name;
			m_Destination = p;
			m_Map = map;
