...
namespace Server.Items
{
    public abstract class BaseShrinker : Item
    {
		...
		{ 
			if ( from.AccessLevel == AccessLevel.Player & !IsChildOf( from.Backpack ) )
			{
			...
			protected override void OnTarget( Mobile from, object target ) 
			{
				...
				else if ( target is BaseCreature )
				{
					...
					if ( Server.Spells.SpellHelper.CheckCombat( bc ) )
						from.SendMessage( "You cannot shrink your pet while its in combat." );
					else if ( bc.Summoned )
						from.SendMessage( "You cannot shrink a summoned pet." );
					else if ( from.AccessLevel == AccessLevel.Player & !from.InRange( bc, 5 ) )
					...
					else if ( !bc.Controlled && from.AccessLevel < AccessLevel.Seer)
						from.SendMessage( "You have to tame the creature first before you can shrink it." );
					else if (( bc.Controlled && bc.ControlMaster == from ) || (from.AccessLevel >= AccessLevel.Seer))
					{
						#region Creation
