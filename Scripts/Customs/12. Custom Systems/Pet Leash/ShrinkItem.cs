...
namespace Server.Items
{
    ...
    public class ShrinkItem : Item
    {
        ...
        public override void OnDoubleClick( Mobile from )
        {
			...
			// This is only check if 'from' is the Owner if 'from' is a Player
        	else if ( from.AccessLevel == AccessLevel.Player & this.Owner != from )
        	{
        		from.SendMessage( "You are not the owner of this pet you cannot claim it." );
        	}
        	else if ( from.AccessLevel == AccessLevel.Player & from.Skills.AnimalTaming.Fixed < bc.MinTameSkill )
        	{
        		from.SendMessage( "You lack the required skill to control this pet." );
        	}
        	else if ( from.AccessLevel == AccessLevel.Player & ( bc.ControlSlots + from.Followers ) > from.FollowersMax )
        	{
