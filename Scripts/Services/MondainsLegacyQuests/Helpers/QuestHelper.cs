...
namespace Server.Engines.Quests
{
    public class QuestHelper
    {
		...
		public static bool FirstChainQuest(BaseQuest quest, object quester)
		{
			...
			try
			{
				int chainID = (int)quest.ChainID;
				if (chainID < 0 || chainID >= BaseChain.Chains.Length)
					return false;

				Type[] chain = BaseChain.Chains[chainID];
				if (chain == null || chain.Length == 0)
					return false;

				return chain[0] == quest.GetType();
			}
			catch (Exception ex)
			{
				Console.WriteLine($"Error in FirstChainQuest: {ex.Message}");
				return false;
			}
		}
