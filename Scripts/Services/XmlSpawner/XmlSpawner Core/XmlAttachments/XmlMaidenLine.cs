...
{
    public class XmlMaidenLine : XmlAttachment
    {
        ...
        public XmlMaidenLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlMaidenLine() { }

        [Attachable]
        public XmlMaidenLine(double refractory, int damage, int range)
        {
        ...
        public void PerformLineBreathAttack(Mobile owner)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                // Iterate over thickness range (-1, 0, 1) to cover three tiles thick line
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int flameHue = 0;  // Adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x1249, 16, 10, flameHue, 0);
