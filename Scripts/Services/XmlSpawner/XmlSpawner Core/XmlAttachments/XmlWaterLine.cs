...
{
    public class XmlWaterLine : XmlAttachment
    {
        ...
        public XmlWaterLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlWaterLine() { }

        [Attachable]
        public XmlWaterLine(double refractory, int damage, int range)
        {
        ...
        public void PerformLineBreathAttack(Mobile owner)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                if (map.CanFit(targetX, targetY, start.Z, 16, false, false))
                {
                    ...
                    int flameHue = 0;
                    Effects.SendLocationEffect(p, map, 0x34FF, 16, 10, flameHue, 0);
