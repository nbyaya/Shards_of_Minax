...
{
    public class XmlStoneBreath : XmlAttachment
    {
        ...
        public XmlStoneBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlStoneBreath() { }

        [Attachable]
        public XmlStoneBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x11B6, 30, 10, 0, 0); // Use a suitable effect for the breath attack
