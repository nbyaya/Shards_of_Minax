...
{
    public class XmlRuneLine : XmlAttachment
    {
        ...
        public XmlRuneLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlRuneLine() { }

        [Attachable]
        public XmlRuneLine(double refractory, int damage, int range)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        Effects.SendLocationEffect(p, map, 0x0E62, 16, 10, flameHue, 0);
