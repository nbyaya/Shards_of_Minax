...
{
    public class XmlTimeBreath : XmlAttachment
    {
        ...
        public XmlTimeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlTimeBreath() { }

        [Attachable]
        public XmlTimeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x1811, 30, 10, 0, 0); // Use a suitable effect for the breath attack
