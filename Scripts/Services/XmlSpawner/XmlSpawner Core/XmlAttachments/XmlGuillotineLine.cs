...
{
    public class XmlGuillotineLine : XmlAttachment
    {
        ...
        public XmlGuillotineLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlGuillotineLine() { }

        [Attachable]
        public XmlGuillotineLine(double refractory, int damage, int range)
        {
        ...
        {
            return String.Format("Guillotine Line Attack: {0} damage over {1} tiles every {2} seconds.", m_Damage, m_Range, m_Refractory.TotalSeconds);
        }
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int effectHue = 0;  // Or adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x1245, 16, 10, effectHue, 0);
