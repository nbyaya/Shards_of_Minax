...
{
    public class XmlGuillotineBreath : XmlAttachment
    {
        ...
        public XmlGuillotineBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlGuillotineBreath() { }

        [Attachable]
        public XmlGuillotineBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x1245, 30, 10, 0, 0); // Use a suitable effect for the breath attack
