...
{
    public class XmlSpikeBreath : XmlAttachment
    {
        ...
        public XmlSpikeBreath(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSpikeBreath() { }

        [Attachable]
        public XmlSpikeBreath(double refractory, int damage, int range)
        {
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        Effects.SendLocationEffect(p, map, 0x11A0, 30, 10, 0, 0); // Use a suitable effect for the breath attack
