...
{
    public class XmlSparkleLine : XmlAttachment
    {
        ...
        public XmlSparkleLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSparkleLine() { }

        [Attachable]
        public XmlSparkleLine(double refractory, int damage, int range)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int sparkleHue = 0; // Or adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x373A, 16, 10, sparkleHue, 0);
