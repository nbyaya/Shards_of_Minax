...
{
    public class XmlSmokeLine : XmlAttachment
    {
        ...
        public XmlSmokeLine(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlSmokeLine() { }

        [Attachable]
        public XmlSmokeLine(double refractory, int damage, int range)
        {
            ...
            for (int i = 0; i < m_Range; i++)
            {
                ...
                for (int thickness = -1; thickness <= 1; thickness++)
                {
                    ...
                    if (map.CanFit(thickX, thickY, start.Z, 16, false, false))
                    {
                        ...
                        int smokeHue = 0;  // Or adjust hue if needed
                        Effects.SendLocationEffect(p, map, 0x3728, 16, 10, smokeHue, 0);
