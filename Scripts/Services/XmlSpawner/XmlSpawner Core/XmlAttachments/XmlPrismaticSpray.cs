...
{
    public class XmlPrismaticSpray : XmlAttachment
    {
        private int m_Damage = 30; // Default damage for the prismatic spray attack
        ...
        private Random m_Random = new Random(); // Random generator for hues
        ...
        public XmlPrismaticSpray(ASerial serial) : base(serial) { }

        [Attachable]
        public XmlPrismaticSpray() { }

        [Attachable]
        public XmlPrismaticSpray(double refractory, int damage, int range)
        {
        ...
        // Serialization methods remain unchanged...
        ...
        {
            return String.Format("Prismatic Spray: {0} damage over {1} tiles every {2} seconds.", m_Damage, m_Range, m_Refractory.TotalSeconds);
        }

        // OnWeaponHit remains unchanged...
        ...
        public void PerformBreathAttack(Mobile owner)
        {
            ...
            foreach (List<Point3D> row in targetRows)
            {
                Timer.DelayCall(delay, () =>
                {
                    ...
                    {
                        int hue = m_Random.Next(1, 3001); // Generate a random hue between 1 and 3000
                        Effects.SendLocationEffect(p, map, 0x3709, 30, 10, hue, 0); // Apply the random hue to the effect
