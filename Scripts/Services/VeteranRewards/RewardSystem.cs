...
namespace Server.Engines.VeteranRewards
{
    ...
    public class RewardSystem
    {
		...
		private static void EventSink_Login(LoginEventArgs e)
		{
			...
			int calculatedLevel = SkillCap + (int)((float)level * SkillCapBonusIncrement);
			if (calculatedLevel > SkillCap + SkillCapBonus)
			{
				calculatedLevel = SkillCap + SkillCapBonus;
			}

			// Only update if the new calculated level is greater than the current skill cap
			if (!Core.SA && SkillCapRewards && e.Mobile.SkillsCap < calculatedLevel)
			{
				e.Mobile.SkillsCap = calculatedLevel;
			}
			else if (!Core.SA && !SkillCapRewards && e.Mobile.SkillsCap < SkillCap)
			{
				e.Mobile.SkillsCap = SkillCap;
			}
			else if (Core.SA && e.Mobile.SkillsCap < (SkillCap + SkillCapBonus))
			{
